...
## 코드 지도 (거대 파일 주의)
- `apps/native_poc/src/native_video_host_main.cpp` **265줄 / main() 123줄**(Phase 0~3.6 + 2-12 완료 2026-08-26 — 브랜치 `refactor/host-split`; 시작/종료는 host_startup_*.cpp 5개 + host_shutdown.cpp(호출 순서는 host_startup.hpp), main loop는 host_stage_*.cpp 14개 + host_loop_helpers.cpp, 컨트롤 세션·캡처·인코더·송신은 각 클래스/멤버로;
  남은 것은 encode_send_h264(989) 분할·순수 클래스 단위테스트·Phase 4 스레드 소유권 재설계) + `host_*.hpp/.cpp` 모듈 50개(startup/shutdown 7 + stage별 15 + 클래스/상태 25 + 공용: log/string_util/args/bgra_scale/
...
- **브랜치**: 리팩터 코드는 `refactor/host-split`(main에서 분기, **Phase 0~3.6 + 2-12 완료 = 6f39ab9**, 다음: 실기 확인 → 버전 범프(0.2.58) → main merge → Phase 4). 계획·이력·체크리스트
