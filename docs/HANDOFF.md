...
## 코드 지도 (거대 파일 주의)
- `apps/native_poc/src/native_video_host_main.cpp` **7,469줄**(Phase 0 완료 2026-08-26 — 브랜치 `refactor/host-split`;
  main() ~7,000줄·지역변수 502개는 Phase 1~3 대상) + `host_*.hpp/.cpp` 모듈 10개(log/string_util/args/bgra_scale/
...
- **브랜치**: 리팩터 코드는 `refactor/host-split`(main에서 분기, **Phase 0 완료 = 3647e41**, 다음 Phase 1). 계획·이력·체크리스트
