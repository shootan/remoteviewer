...
설치본은 `dist/GNLinkSetup-0.2.58.exe`(리팩터 브랜치 `refactor/host-split` 빌드 9d37c9e, **실기 확인 대기**; 리팩터 전 설치본 = 0.2.57).** 남은 것은 실기 검증 대기 + P2/P3.
...
- **브랜치**: 리팩터 코드는 `refactor/host-split`(main에서 분기, **Phase 0~3.6 + 2-12/2-13/T1~T4 완료 = 89211b6**, 다음: 0.2.58 설치본 실기 확인 → main merge → Phase 4). 계획·이력·체크리스트
