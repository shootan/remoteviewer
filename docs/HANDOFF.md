...
## 코드 지도 (거대 파일 주의)
- `apps/native_poc/src/native_video_host_main.cpp` **7,795줄**(Phase 0+1 완료 2026-08-26 — 브랜치 `refactor/host-split`; 상태 struct 12개가 파일 스코프에, main() 공유 지역변수 502→77;
  main() ~6,700줄은 Phase 2~3 대상) + `host_*.hpp/.cpp` 모듈 10개(log/string_util/args/bgra_scale/
...
- **브랜치**: 리팩터 코드는 `refactor/host-split`(main에서 분기, **Phase 0+1 완료 = 90759ae**, 다음 Phase 2 클래스화). 계획·이력·체크리스트
