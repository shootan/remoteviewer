...
2026-09-07 후속 작업: [구현 전 크로스체크 및 Windows 복구 경로 수정 지시](windows_viewer_recovery_work_order.md)를 기준으로 독립 확인 후 구현한다. 이 분석의 확정 결함과 현장 원인 미확정을 구분하며, 실제 Windows 실행 경로의 통합 검증을 완료 조건으로 삼는다.
