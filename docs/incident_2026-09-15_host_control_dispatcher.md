...
## 재발 시 안전한 우회 (조건부)
증상이 "연결됨 → 화면 선택 창 → 15초 내 끊김, 재시작해도 반복"이면 **GNLink 재시작이 아니라 호스트 PC의 응답 없는 창(GPU 렌더링 앱·오류 대화상자·이번엔 LDPlayer)을 닫는다.** 15:52:29 실측대로 재시작 없이 즉시 복구된다. 어느 창인지는 로그가 알려주지 않는다. 재부팅·0.2.127 업데이트는 이 결함을 고치지 않는다.
