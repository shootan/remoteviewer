...
- 검증 전용(추가 코드 작업 없음):
  ...
  - [ ] Android Phase E live control verify (`Refresh`, `Desktop Mode`, `window select`, host/UI 상태 반영)
  - [ ] Android valid-capture-source verify (`CreateForWindow(GetShellWindow())` fallback 없는 host 환경에서 재검증)
