...
- 검증 전용(추가 코드 작업 없음):
  - [ ] M7 1080p30/720p30 Pass 로그 5회 확보 (현재 1080: 0/5, 720: 5/5)
  - [x] M7 실패 케이스/회귀 로그 정리 (스모크/튜닝 8건 수집 + 자동 판정 아이콘 출력 추가)
  - [ ] M7 기본 실행 프로필 확정 (720p 후보 확정: `5Mbps/keyint60/h264NoPacing=1/frameGatingDisable=1`, 1080p 고정값 검증 잔여)
...
- 검증:
  - [ ] 1080p30/720p30 각각 Pass 로그 5회 확보 (현재 1080: 0/5, 720: 5/5)
  - [x] 체크리스트 항목별 실패 케이스/회귀 로그 정리 (스모크/튜닝 8건 수집 + `M7_STATUS_ICON`/`M7_STATUS_REASON` 자동 출력)
  - [ ] 기본 실행 프로필 확정 (720p 프로필 고정 완료, 1080p 고정 프로필 검증 잔여)
  - 완료조건:
