...
- 검증 전용(추가 코드 작업 없음):
  ...
  - [x] 외부 2PC 테스트 준비물 정리 (native/web 각 external bundle + quickstart 포함, native는 one-command host/client wrapper 포함, 기본 smoke profile은 fixed 30fps, 최신 native 2PC GUI profile은 input/control on)
