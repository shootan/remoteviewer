...
- [ ] MF H264 인코더 async 이벤트 폴링 축소/비동기화 제어 액션:
...
7. [ ] 병목 원인 기반 `M1/M2/M2.5/M3` 순서 재배치 및 토글 기본값 재정의
...
9. [ ] `REMOTE60_NATIVE_H264_ASYNC_POLL_*` 토글 기반 재측정 실행:
   - `sendDoneUs`가 `queueToSend` 중심으로 이동하는지 확인.
   - 필요 시 `async poll sleep` 기본값을 0~1ms 구간으로 고정.
10. [ ] 1080p3개 장면 스모크에서 `LAT_P95_US` 70ms 이하 달성 여부 확인
