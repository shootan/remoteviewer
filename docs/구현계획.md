...
- 검증:
  - [ ] AMD/NVIDIA/Intel별 backend 고정 표기 + fallback 동작 실측 검증 (2026-03-07 1차 자동검증/콘솔 세션 재검증/크래시 재현점검 결과 동일: `mft_hw`는 반복 5회 `OVERALL_OK`, `nvenc_hw/qsv_hw`는 `backend unavailable`로 host init fail)
