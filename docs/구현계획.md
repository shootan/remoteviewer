...
- 코드 작업 필요:
  ...
  - [x] Android settings tab + runtime quality control (`Windows/Desktop/Settings`, host bitrate/fps runtime apply without host process restart)
  - [x] Android settings persistence + viewer-only host streaming (saved bitrate/fps auto-apply on connect, list scene에서는 host stream stop)
...
- 검증/판정 필요 (코드 구현 완료 또는 부분 완료):
  ...
  - [ ] Android direct timestamp/QPC overflow LDPlayer closeout (`desktop/window select`에서 `video_debug out` 증가, `select_timeout` 해소, 실기기 재확인)
...
- 검증 전용(추가 코드 작업 없음):
  ...
  - [ ] LDPlayer2 desktop DXGI live verify (`current RDP session: dxgi_no_output_found -> WGC fallback`, reconnect instability 재확인 필요)
