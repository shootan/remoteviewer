...
- 코드 작업 필요:
  ...
  - [ ] Android Phase D full-frame 렌더 보정 (`TextureView` 일부만 보이는 상태 해소, sizing/transform 정리)
  - [ ] Android Phase E session/JNI 제어 브리지 추가 (`window list snapshot`, `refresh/select`, `Desktop Mode` 요청)
  - [ ] Android Phase E 레퍼런스 탭 UI 1차 구현 (`LD플레이어` window list, `디바이스` monitor list, selected target 상태)
...
- 검증 전용(추가 코드 작업 없음):
  ...
  - [ ] Android Phase D final visual verify (`surface buffer=1234x720`, `video=1234x720`, screenshot/실기기 full-frame 확인)
