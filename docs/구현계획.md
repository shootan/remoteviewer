...
- 검증/판정 필요 (코드 구현 완료 또는 부분 완료):
  ...
  - [ ] M3.5 background 입력 주입 1차 수동 검증 (`LDPlayer` desktop click pass, keyboard text commit pending)
...
- 검증 전용(추가 코드 작업 없음):
  ...
  - [ ] Android Phase F soft keyboard runtime verify (`keyboard button` + `IME served view` pass on `LDPlayer`, `committed text`/backspace/enter pending)
