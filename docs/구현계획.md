...
- 코드 작업 필요:
  - [ ] M4 AMD 기본화/안정화 마무리 (`amf_hw` 경로는 존재하나 기본 정책/실측 고정 미완)
  - [ ] M6 FEC/NACK/RTX 중 채택안 설계 + 구현(채택 시)
...
- 검증 전용(추가 코드 작업 없음):
  - [ ] M7 1080p30/720p30 Pass 로그 5회 확보
  - [ ] M7 실패 케이스/회귀 로그 정리
  - [ ] M7 기본 실행 프로필 확정
...
- 코드 작업:
  ...
  - [ ] (조건부) 최소 FEC 또는 NACK/RTX 채택안 구현
- 검증/설계:
  - [ ] FEC/NACK/RTX 필요성 판정(손실 패턴/복구시간 기준)
