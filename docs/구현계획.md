...
- [ ] 입력 주입 모드 `background_message` 추가 (`SendInput`/`SetCursorPos` 금지)
- [ ] 마우스 포인터 비이동 보장(실제 OS 커서 위치/이동 미변경)
- [ ] 클릭/드래그/키보드 입력 이벤트만 주입(마우스 이동 단독 이벤트 기본 미주입)
- [ ] 가려진 윈도우 포함 지정 타겟 HWND 직접 입력 주입 경로 구현
- [ ] JSON 설정 추가:
...
## 5) 앞으로 실행 순서
0. [ ] `M3.5 Background 입력 주입` 구현/검증 완료 (최우선)
