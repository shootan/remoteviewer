...
### 미완료 항목 빠른 필터 (2026-03-07 코드+1차 검증 대조 기준)
- 코드 작업 필요:
...
- 검증/판정 필요 (코드 구현 완료 또는 부분 완료):
  ...
  - [ ] M4 backend auto/fallback 로그 + 성능 완료조건 검증
  - [x] M5 정적 장면 MBPS 절감 검증
  - [ ] M5 frame gating 완료조건 검증(`MBPS_AVG`, 화질/응답성)
- 검증 전용(추가 코드 작업 없음):
  - [ ] M7 1080p30/720p30 Pass 로그 5회 확보 (현재 1080: 0/5, 720: 0/5)
  - [x] M7 실패 케이스/회귀 로그 정리 (스모크/튜닝 8건 수집 + 자동 판정 아이콘 출력 추가)
  - [ ] M7 기본 실행 프로필 확정 (블로커 해소됨: `capture_input_stall` 제거 + `GATE_A_PASS=True`; 잔여: `M7_TARGET_DECODED_FPS(28)` 미달)
...
- 검증:
  ...
  - [ ] AMD/NVIDIA/Intel별 backend 고정 표기 + fallback 동작 실측 검증 (2026-03-07 후속 패치로 `nvenc_hw/qsv_hw` 요청 시 `mft_enum_hw` 자동 폴백 + `backendFallbackReason=requested_backend_unavailable` 확인, 다만 NVIDIA/Intel 실장비에서의 전용 backend 고정 검증은 미완료)
  - [ ] 동일 장면에서 generic MFT 대비 fps/latency/mbps 중 2개 이상 개선 검증
  - 완료조건:
    - 백엔드가 로그에 명확히 고정 표기됨
...
- 검증/설계:
  ...
  - [ ] 채택안 적용 시 `PRESENT_GAP_OVER_1S=0` 유지 + 손실 구간 복구시간 단축 검증 (환경 블로커: 최근 시퀀스 `MBPS_AVG=0` 구간으로 절대 성능 판정 보류)
  - 완료조건:
...
- 검증:
  - [ ] 1080p30/720p30 각각 Pass 로그 5회 확보 (현재 1080: 0/5, 720: 0/5)
  - [x] 체크리스트 항목별 실패 케이스/회귀 로그 정리 (스모크/튜닝 8건 수집 + `M7_STATUS_ICON`/`M7_STATUS_REASON` 자동 출력)
  - [ ] 기본 실행 프로필 확정 (블로커 해소됨: `capture_input_stall` 제거 + `GATE_A_PASS=True`; 잔여: `M7_TARGET_DECODED_FPS(28)` 미달)
  - 완료조건:
