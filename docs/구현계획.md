...
- 코드 작업 필요:
  ...
  - [x] native window GUI parity v1 (`Desktop Mode`/window list/선택 상태/native window-target 입력/touch 단일포인터)
...
- 검증 전용(추가 코드 작업 없음):
  ...
  - [x] 외부 2PC 테스트 준비물 정리 (native/web 각 external bundle + quickstart 포함, 기본 smoke profile은 fixed 30fps, native window-input template/web GUI path 분리)
