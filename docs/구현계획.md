...
- 코드 작업 필요:
  ...
  - [x] Android settings tab + runtime quality control (`Windows/Desktop/Settings`, host bitrate/fps + desktop backend runtime apply, stable default=`WGC`)
...
11. [ ] Window capture 체감 렉 저감 1차: capture callback에서 `CopyResource`만 수행하고 `Map/readback`은 별도 worker/staging ring으로 분리
12. [ ] Window capture 품질/프레임 2차: client crop/resize/BGRA->NV12 경로 GPU화로 readback 횟수와 복사 바이트 수 축소
