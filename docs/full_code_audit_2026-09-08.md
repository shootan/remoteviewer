...
| I29 | P2, 업데이트 해소됨 (2026-09-09, history #477) | Host 가 bootstrap 종료의 의미만 고쳐서는 부족하다 — **copy 가 도중에 죽으면 timeout 전체를 기다렸다.** working copy 가 `<stem>.alive` 뮤텍스를 보유하고 Host 가 함께 기다린다(뮤텍스인 이유: **죽으면 abandoned 로 풀린다** — 정리 코드를 못 돌린 죽음이 잡아야 할 죽음이다) | 실측 **328ms** 에 감지, 사유도 분리. 반대 증거: 끄면 **20000ms** 를 다 기다리고 사유가 뭉개진다 |
