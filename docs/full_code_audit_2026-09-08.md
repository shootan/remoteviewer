...
| I12 | P1, 업데이트 해소됨 (2026-09-09, history #470) | `RelaunchConfig::isStillRunning` 이 비면 **"전부 나갔다"** 로 가정했고 **생산은 이 자리를 한 번도 채우지 않았다**. 출시본에서 생존 검사는 꺼져 있었고, 그것을 덮던 테스트는 **아무도 안 돌리는 구성**을 덮고 있었다 | 답을 셋(`Running/Exited/Unknown`)으로 바꾸고 비면 실제 검사를 쓴다. 일반화로 `UpdateEffectsConfig::unwired()` + `run()` 의 거부를 넣어 **비어 있는 seam 으로는 아무것도 실행되지 않는다** |
| I13 | P1, 업데이트 해소됨 (2026-09-09, history #470) | `Commit()` 이 `DeleteFileW` 결과를 안 보고 `movedAside_` 를 비웠다. 지워지지 않은 백업은 **어디에도 흔적이 없었고**, 그 이름을 차지한 채 **다음 업데이트의 move-aside 를 막아 설치 자체를 불가능하게** 만든다 | 결과를 읽어 이름+오류코드로 기록. 지울 수 없으면 `<name>.gnlink-old.N` 으로 옮겨 이름을 비운다(**실행 중인 파일도 rename 은 된다**). 격리 재현 + 역대조 포함 |
| I14 | P1, 업데이트 해소됨 (2026-09-09, history #470) | 롤백이 **자기가 띄운 프로세스를 못 멈춘 상태에서도** 파일을 되돌렸다. 절반만 복구된 설치가 남고, 실패 원인은 롤백 탓으로 읽힌다 | `stopStarted` 가 `unstoppable` 을 보고하고, 비지 않으면 **파일에 손대기 전에 거부**한다. 셸 경유는 launch 별 1:1 상관관계가 설 때만 소유를 주장한다 |
| I15 | P2, 테스트 하네스 해소됨 (2026-09-09, history #470) | 시나리오 쓸어담기가 image 경로를 읽으려 `OpenProcess` 를 먼저 했고 **열 수 없는 프로세스를 건너뛰었다** — 건너뛴 것이 **정확히 멈추려던 프로세스들**이었다. 스냅샷에는 이름으로 계속 보였다. 매 실행 넷이 살아남아 다음 시나리오가 시작 바이트조차 쓰지 못했고, **프로세스는 하나도 안 보이는 채로** 바이트 단정이 실패했다 | 스냅샷 이름으로 식별하고, 경로는 **확인되면** 좁히되 **확인 못 하면 포함**한다. 못 여는 것이야말로 놓치던 대상이다 |
| I16 | **P0, 테스트 하네스 해소됨** (2026-09-09, history #471) | 시나리오 스위트가 `%TEMP%` 의 `gnlink-scn-*` 를 **열거해 `remove_tree`** 했다 — **저장소 root 밖 삭제**, 최상위 규칙 위반. 테스트 실행 승인에 포함되지 않는 행위다 | root 를 빌드에서 주입(`${CMAKE_SOURCE_DIR}/.claude/scenario-runs`), **자기 run 디렉터리만** 가드 통과 시 삭제, 남의 잔재는 목록만 |
| I17 | **P0, 테스트 하네스 해소됨** (2026-09-09, history #471) | 쓸어담기가 경로를 못 읽으면 **`mine = true`** 로 두고 **이름만으로 Terminate** 했고, 시작 시 **빈 scope** 로 호출됐다. **같은 스위트의 동시 실행을 죽였을 것**이고 PID 재사용 위험도 그대로 | 경로 확인 필수, 미확인은 **보고만**. 반례 3종(같은 이름 다른 경로 / 동시 run / `%TEMP%` 제외) |
| I18 | P1, 업데이트 해소됨 (2026-09-09, history #471) | 셸 경유 실행의 **스냅샷 차분 1개** 를 소유 근거로 썼다. 사용자가 같은 프로그램을 그 순간 켜면 차분도 1이고, **그 위에서 연 handle 은 남의 신원을 고정**한다 — 롤백이 그걸 죽인다 | 스냅샷 비교 삭제. **`CreateProcessWithTokenW`** 로 실제 handle 확보, 실패 시 **소유 주장 안 함**(`ownershipUnknown`) → 강제 종료·롤백 진입 없음. 시나리오 7 + 역대조 |
| I19 | P2, 업데이트 (2026-09-09, history #471) | `.gnlink-old` 를 옮겨 이름을 비우는 것이 **만능이 아니다**: 공유 없이 열린 핸들이 잡은 파일은 **삭제도 rename 도 안 된다**. rename 이 구제하는 건 **실행 중 이미지**뿐 | 상한 50 + 초과 시 **원인을 증상과 함께** 보고. 세 경우(실행 중 이미지 / 상한 초과 / 배타 핸들)를 각각 단정으로 고정 |
| I20 | P1, 업데이트 해소됨 (2026-09-09, history #473) | 재실행이 한 단계라, **롤백이 가능한 구간에 소유 불가 프로세스(셸로 뜬 클라이언트)가 존재**할 수 있었다. 그러면 그 시도의 롤백이 **영구히 막힌다**(실측: 순서를 되돌리면 시나리오 7 이 `RollbackFailed`) | **Required → health → commit → Optional** 로 분리. 되돌릴 것이 없어진 뒤에만 소유 불가를 허용. 회귀 7종 + 제품 경로 시나리오 7·9 |
| I21 | P2, 업데이트 해소됨 (2026-09-09, history #473) | 단계 분리 직후 `lastOutcomes` 를 **단계마다 비워서** required 기록이 optional 단계와 함께 사라졌고, 그 뒤 읽는 쪽이 **빈 벡터를 인덱싱**했다(실측 segfault) | 시도 단위 누적으로 변경. 로그는 새로 늘어난 것만 |
| I22 | P2, 테스트 하네스 해소됨 (2026-09-09, history #473) | 시나리오 노브 `oldHealthOk` 가 **한 번도 발화하지 않았다** — `rolledBack` 을 아무도 true 로 만들지 않아 복원된 빌드도 `newHealthOk` 로 판정. `RestoredButUnhealthy` 기대 케이스들이 **테스트하지 않는 이유로** 통과 | 두 번째 required 단계를 롤백 신호로 연결. 깨끗한 `RolledBack` 이 이제 만들어진다 |
| I23 | **P0, 테스트 하네스 해소됨** (2026-09-09, history #474) | 시나리오 스위트가 셸로 launch 를 발사한 뒤 **디렉터리를 지웠고**, 탐색기가 뒤늦게 실행하면서 **사용자 화면에 "찾을 수 없습니다" 대화상자**가 떴다. `GetFileAttributes` 선확인은 **TOCTOU 라 방어가 아니다** — 확인·삭제·실행이 이 순서로 일어난다 | 자기 종결 fixture(`remote60_scn_dummy`, witness 기록 후 종료) + **발사한 시나리오 안에서 witness 대기** + 끝내 안 오면 **디렉터리 보존**. `update_relaunch_test` 도 같은 처치 |
| I24 | P2, 테스트 하네스 해소됨 (2026-09-09, history #474) | 정리 거절 메시지가 "보류 중" 과 "root 밖" 을 같은 `else` 로 묶어, **root 안 디렉터리를 두고 "root 밖이라 못 지운다"** 고 찍었다 | 사유 분리. **틀린 이유를 대는 메시지는 없느니만 못하다** |
| I25 | **P1, 업데이트 해소됨** (2026-09-09, history #475) | 조합이 `Global\GNLinkUpdate` 를 상수로 박아, **이 조합을 돌리는 모든 테스트가 설치된 제품과 같은 기계 전역 뮤텍스를 두고 경쟁**했다(실측: 한 실행이 lock 때문에 `NothingToDo`). 반대 방향이면 **테스트가 진짜 업데이트를 막는다** | `UpdaterDeps::lockName` 으로 주입화, `validate()` 가 요구. 생산은 기계 전역, 테스트는 `Local\...-<pid>`. **생산이 여전히 진짜 이름을 공급하는지**를 조합 테스트가 단정 |
| I26 | P2, 테스트 하네스 해소됨 (2026-09-09, history #475) | fixture 를 자기 종결형으로 바꾸자 **"실행 중 이미지를 잡고 있다" 던 통제의 victim 도 즉시 종료**해, `DeleteFileW` 실패 단정이 타이밍으로 통과했다 | 그 통제에만 종료하지 않는 사본을 쓴다. **고치는 변경이 다른 곳의 전제를 깨뜨렸는지**를 같이 봐야 한다 |
| I27 | **P0, 업데이트 해소됨** (2026-09-09, history #477) | Host 가 handoff 대기에서 **bootstrap 핸들**을 지켜봤다. bootstrap 은 자기 실행 파일이 교체 대상이라 **즉시 종료하는 것이 정상**인데, Host 는 그것을 죽음으로 읽고 **"설치된 버전 그대로"** 를 알린 뒤 ready 를 닫았다. copy 는 `signalReady` **반환값을 안 보고** 진행해 **그 Host 를 종료**했다 — 사용자는 취소를 듣고 기계를 잃는다 | `HandoffStep` 에 **KeepWaiting** 추가 + **exit code 판독**, **ack 이벤트**로 양방향 handshake, `signalReady` 가 bool, `awaitAck` 신설(**둘은 함께**), `PrepareForSwap` 에서 판정. **실제 3프로세스 회귀** 5종 + 반대 증거 |
| I29 | P2, 업데이트 해소됨 (2026-09-09, history #477) | Host 가 bootstrap 종료의 의미만 고쳐서는 부족하다 — **copy 가 도중에 죽으면 timeout 전체를 기다렸다.** working copy 가 `<stem>.alive` 뮤텍스를 보유하고 Host 가 함께 기다린다(뮤텍스인 이유: **죽으면 abandoned 로 풀린다** — 정리 코드를 못 돌린 죽음이 잡아야 할 죽음이다) | 실측 **328ms** 에 감지, 사유도 분리. 반대 증거: 끄면 **20000ms** 를 다 기다린다. ⚠️ **범위**: 뮤텍스 확보 **전** 사망은 timeout 경로이고, 뮤텍스는 **보유 스레드 수명**이라 main-thread 보유 계약 위에서만 프로세스 수명과 같다 |
| I30 | **P0, 업데이트 해소됨** (2026-09-09, history #478) | Host 가 `SetEvent(ack)` 직후 핸들을 닫는데 worker 는 **`PrepareForSwap` 에 가서야** 열었다 → 마지막 핸들이 닫히면 **named object 소멸** → 승인이 전달됐는데도 open 실패로 abandon, 그런데 **Host 는 이미 나가는 중.** ⚠️ **반환 bool 은 전부 맞았고 핸들이 없어졌다** — bool seam 검사로는 안 잡힌다 | worker 가 **ready 전에 채널을 열어 보유**(3-seam, `validate()` 가 넷 다 요구). 못 열면 **신호를 보내지 않는다.** Host 는 `SetEvent` 실패 시 **`ExitNow` 금지**. 회귀를 **실제 handle 수명**으로: 보유/역예시(늦게 열기)/채널 없음 |
| I31 | P1, 업데이트 해소됨 (2026-09-09, history #478) | `deadline = GetTickCount() + timeoutMs` 가 **32bit 합**이라 **49.7일 가동에서 wrap** → 모든 대기가 즉시 timeout. `WAIT_FAILED` 도 종결이 없어 남은 시간을 스핀 | `remaining_ms()` 로 분리해 **64bit 뺄셈** + **시계 주입**, 경계값 단위 회귀. `WAIT_FAILED` 명시 종결 |
| I28 | P1, 업데이트 해소됨 (2026-09-09, history #477) | Host 의 대기 루프가 **lambda 안에** 있어 **제품을 실제로 업데이트하지 않으면 돌릴 수 없었다.** 그래서 한 번도 안 돌았고, 그래서 틀린 채로 있었다 | `await_handoff` 로 분리. 테스트가 **그 함수를** 부른다 — 재현본을 시험한 것이 결함이 남은 이유였다 |
