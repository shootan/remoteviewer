...
- 확인: (1) 이번 세션은 **직접 연결**(`directory chose 175.207.45.151:43000 (public)`, 릴레이 bound 없음) — 14시 릴레이 세션과 다르나 요인 미확정. 제어 RTT med 2.0ms, p95 4.5~10.2ms, max 20ms. (2) 영상 구간 뷰어 recv=decoded 50.5/s(근사 구간), present gap med 16.1ms p95 46.9ms, avgLatency(정렬 시계) med 8.4ms; 타이핑 5.8/s·present gap med 218ms·avgLatency med 113ms(희소 콘텐츠·인코더 보유 설계와 부합, 미확정); 뷰어 decode 5ms·decode→present 5~9ms. (3) **NACK 협상 실기 확인**(`nackNegotiated=1`, `nackOn=1`)이나 청크 310,337개 손실 0(`nackSent=0`) → 재전송 효과 미입증. (4) **완성 IDR 무요청 재동기 실기 확인**: seq gap IDR 57건 전부 `keyResync`, `keyReq=0`, 호스트 도달 요청은 시작 reason=7 1회 + UAC reason=1 2회(09-05 의 90/90 대비 폭주 없음). stale/holdLatest 0, liveness/peer-lost 없음. (5) UAC(15:15:56 wgc→15:15:59.6 dxgi): Congested 2회(reason=decode_queue, lag 567/848ms, 각 IDR 1회로 0.3~0.6s 내 normal) — 호스트 backend 전환 첫 프레임의 `sendQpcUs−captureQpcUs` 가 126ms/**820ms** 로, 전환 직후 실제 프레임의 capture stamp 지연이 뷰어에 decode 적체로 읽힌 것(3단계 synthetic 시계 문제와 별개, 원인 미확정 → 신규 항목 P11). 사용자 체감 없음. (6) UAC 클릭 좌표: secure_input 에이전트의 `inject landed` 진단이 프로세스당 12개 버튼 이벤트만 기록(14:27 에 소진)해 15:15 클릭은 로그 없음 → 판정 불가, P9 미해결 유지.
- 잔여: NACK 재전송 효과(손실 필요), 14시 릴레이 세션 RTT, 호스트 전수 capture→wire, P2P 성립 요인, UAC 좌표(진단 예산 재충전 계측 필요), 5분 장기, P10·P11 코드 수정.
