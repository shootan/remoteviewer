...
**🔴 여전히 없는 것 — ④ 의 "실제 UI 응답 경계 검사"**
대기를 스레드로 옮긴 것은 코드로 확인되지만, **UI 가 그 사이 응답한다는 단정은 어디에도 없다.** 지금 보증은 *"인라인이 아니다"* 이지 *"막히지 않는다"* 가 아니다 — 이 세션이 내내 구분해 온 그 차이다. 단정하려면 셸의 창과 메시지 루프를 테스트가 돌려야 하고, 그 수단이 없다. **미검증으로 둔다.**
