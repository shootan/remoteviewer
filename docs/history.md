...
**아직 안 된 것**: **서명 미실행** — 서명 스크립트가 권한 게이트에서 차단됐다. 사용자 승인 후 `windows.sig` 생성·자체 검증·3중 검증을 하고, 그 다음이 NAS 인계다.
**하지 않은 것**: 게시·설치·라이브 실행·프로세스 중지·캐시 삭제·push·NAS 변경 없음. APK 재생성 없음.
