...
### 507) 2026-09-11 `0.2.110` 게시 — **업데이터가 처음으로 자기 자신을 담아 나간다**
1~7 이 전부 들어간 Windows 단독 후보. **Android 무변경**(APK 파일을 하나도 건드리지 않았다).

**바뀐 바이너리 — 수정이 실제로 들어갔다는 증거**
| | 0.2.109 | 0.2.110 |
|---|---|---|
| `GNLinkUpdater.exe` | 532,480 | **538,624** `0ac7aea84d5268aa…` ← 종료·relaunch 수정이 여기 있다 |
| `GNLinkHost.exe` | 660,480 | 660,480 `cfefaaefd20ec909…` |
| `GNLinkStream.exe` | 1,000,448 | 1,000,448 `a23a656cd2518799…` |
| `GNLinkSetup.exe` | 4,352,000 | **4,358,144** `3f0c3f5ca6b3fa42…` |
manifest **1,687B `7c030901b2c45645…`**(artifact **10줄**) · sig 파일 `142e0a7e1afccdcc…`

**⭐ 이번 manifest 는 목록을 스스로 적지 않는다**
`make_release_manifest_110.py` 는 이름을 **`product_payload_names()` 에서 읽는다.** 0.2.109 가 `GNLinkSetup.exe` 없이 나간 이유가 *"이 스크립트가 자기 사본을 들고 있었고 그 사본이 틀렸다"* 였다. 7번(게시 전 차집합 거부)이 반대편을 막고, 이건 같은 구멍을 **애초에 만들지 않는다.** 목록의 출처가 하나다.

**게시 검증**
- preflight 에서 **7번 게이트 통과**(`the manifest names everything the update replaces`)
- 제품의 C++ `default_verifier()` 로 **실제 문서** 검증 Ok / 1바이트 변조 SignatureInvalid
- manifest ↔ 디스크 10/10 · 서버 10파일 일치 · **공개 URL 10/10 익명 https 해시 일치**
- **재시작 0 · env 0 · 기존 버전 디렉터리 무변경**

**⭐ 백업 가드가 실제로 값을 했다**
```
a backup of 0.2.109 already exists; this one goes to 0.2.109-20260910T151705Z
backed up 0.2.109:  1d5c11b5…(r2 manifest)  80203ff6…(r2 sig)
```
`manifest-backups/0.2.109/` 의 **원본(최초 게시분)은 그대로 남고**, 이번에 교체되는 r2 짝은 타임스탬프 경로로 갔다. 가드가 없었다면 **롤백 지점이 r2 로 덮여** 최초 게시분으로는 돌아갈 수 없었다.
- ⚠️ **다만 "원본이 보존됐다" 를 "좋은 것이 보존됐다" 로 읽으면 안 된다.** 그 원본이 바로 **현장을 깨뜨린 문서**다 — #510 참조.

**🔴 게시하면서 또 한 건 — dry-run 이 하지도 않은 검사를 "전부 통과" 라고 보고했다**
`verify_on_server` 가 dry-run 에서 아직 없는 파일을 **건너뛰고도** 마지막에 *"all 10 artifacts on the server match"* 를 찍었다. **10개를 하나도 안 보고 10개가 맞다고 말한 것**이다.
→ **검사한 개수**를 세어 보고하도록 고쳤다: `0 of 10 artifacts checked on the server; 10 not present yet (dry run)`.
⚠️ **요약 줄이 방금 한 일을 설명하지 않으면 그건 요약이 아니다.** 이번 라운드에서 같은 형태를 세 번째 잡았다(테스트 detail 이 다른 시점을 읽은 것, 서명 변조가 변조하지 않은 것, 그리고 이것).

**버전 선택**: 검증용이 *"게시 후보 진행하세요"* 라고 두 번 승인했고 번호는 **다음 순번**이라 `0.2.110` 으로 했다. **Android 는 올리지 않았다** — 바뀐 것이 없는데 번호를 올리면 그 번호가 아무것도 증명하지 않는다(0.2.109 와 같은 판단).

**⚠️ 사용자에게 반드시 전할 순서 (검증용 요구)**
**새 버전을 게시해도 현재 설치된 0.2.108 업데이터로는 이 수정이 실행되지 않는다.** 그 업데이터에는 수정이 **하나도 없고**, 업데이터는 지금까지 **어떤 릴리스로도 교체된 적이 없다**(5번). **수동 설치 1회가 선행돼야** 그 다음부터 인앱 업데이트가 이 코드로 돈다. **그전까지 인앱 재시도 안내 금지.**

**남은 미검증**: 실기 완주 · 언인스톨 항목 실제 갱신 · **설치된 업데이터와 동일 조건의 10파일 업데이트**(bootstrap 선행 필요) · `gdi_capture_process`(#506, 별건).

### 508) 2026-09-11 `gdi_capture` 원인 가설 **세 개가 전부 기각**됐다 — 그리고 문턱이 분산 안에 있다
검증용이 세 번째 설명을 냈다: *"Debug 바이너리를 fps 임계값에 대고 쟀다"* (Debug 42.99/46.99 FAIL, Release 54.33/56.66 PASS). **내 측정으로는 성립하지 않는다.**

**6회 측정 — GNLink 0개, 콘솔 세션, 같은 커밋, config 만 다름**
```
Debug   run1 rc=0 fps=59.3321 copyAvg=16873    Release run1 rc=0 fps=59.3254 copyAvg=16856
Debug   run2 rc=0 fps=59.3114 copyAvg=16914    Release run2 rc=0 fps=58.3117 copyAvg=17196
Debug   run3 rc=0 fps=58.6658 copyAvg=17025    Release run3 rc=0 fps=57.3320 copyAvg=17502
```
- **양쪽 전부 통과**하고, **Debug 가 오히려 미세하게 빠르다.** config 로는 `42.99 → 59.33` 을 설명할 수 없다.
- 가설 세 개가 순서대로 나왔고 셋 다 내 쪽에서 재현되지 않았다: **live 호스트 경합**(#505, 철회) → **원인 미규명**(#506) → **Debug config**(이번, 기각).

**모든 실행에 걸쳐 살아남는 관찰은 하나뿐이다**
`1/copyAvg ≈ 측정 fps` 가 **양쪽 사람, 양쪽 config, 모든 실행**에서 소수점까지 맞는다. **상한은 화면 갱신률이 아니라 복사 시간**이고, 그 값이 실행마다 **16.8ms ~ 23.3ms** 사이에서 움직인다.
⚠️ **50fps 문턱은 정확히 20.0ms 다 — 관측 분산의 한가운데.** 그러면 이 스위트의 판정은 코드가 아니라 **그 순간의 부하**가 정한다. 검증용의 실패 실행이 어떤 부하였는지는 **이제 재구성할 수 없다.**
- 별개 관찰(판정을 뒤집는 요인은 아님): 같은 복사를 **부모는 1.8~2.5ms, 격리 캡처 프로세스는 17~23ms** — 약 10배 격차가 모든 실행에서 일관된다.
- ⚠️ **"원인 미규명" 도 "config 탓" 도 아니고, "문턱이 분산 안에 있다" 가 실제로 확인된 것이다.** 세 번 연속으로 *그럴듯한데 확인되지 않은 설명* 을 붙였다 — 두 사람 모두. **재현되지 않는 설명은 설명이 아니다.**

### 509) 2026-09-11 `udp_control_e2e` — Release 전용이 아니라 **상태 의존**이다
검증용 Release sweep 에서 `udp_control_e2e` rc=1 · `viewer_udp_recovery` rc=127 을 보고했다.

**내 Release 전량 재빌드 후 sweep**: **64 스위트 `^PASS` 1803 · 실패 1건**(`udp_control_e2e` 1P/17F). `viewer_udp_recovery` **rc=127 은 재현되지 않았다**(통과).

**그런데 config 문제가 아니다** — 연속 3회:
```
Release  rc=1 PASS=1 FAIL=17     Release  rc=1 PASS=1 FAIL=17     Debug  rc=1 PASS=1 FAIL=17
```
- **Debug 도 똑같이 실패한다.** 그런데 몇 시간 전 내 Debug sweep 에서는 **18 PASS 로 통과**했다.
- 실패 지점은 `native_video_client_tcp_control.cpp:202` **`udp hello ack failed`** — 세션이 아예 성립하지 않는다. 나머지 17건은 그 결과다.
- 검증용도 이전에 **1P/17F → 16P/2F** 로 오가는 것을 봤다. **같은 두 상태를 오간다.**
- ⚠️ **config 가 아니라 기계 상태에 달려 있다.** 무엇이 바뀌면 성립/불성립이 갈리는지는 **확인하지 않았다** — UDP 포트 점유가 후보지만 **추정이고 단정하지 않는다.** 이번 3커밋과는 무관하다(건드린 파일에 udp control 관련 0개).
- 같은 시점에 좀비 테스트 프로세스 2개(`viewer_udp_recovery`, `winhttp_tls_posture`)를 정지시켰다 — Release 링크를 막고 있었다. **스위트가 프로세스를 남긴다는 것 자체가 별건 결함**이다.

**총합의 재현 조건은 이제 셋이다**: (1) 스위트 출력 형식(5개는 `^PASS` 0), (2) **config**(Debug 1820 / Release 1803), (3) **빌드 최신성**. 셋을 밝히지 않은 총합은 재현되지 않는다. `.claude/sweep-per-suite*.txt` 에 **config·빌드 시각을 헤더로** 넣었다.

### 510) 2026-09-11 🔴 **롤백 이름이 고장난 문서를 가리킨다** — 그리고 이번 라운드의 반복 형태
검증용이 0.2.110 게시를 **OK** 하면서 별건 하나를 찾았다. 서버에서 직접 확인했다:
```
manifest-backups/0.2.108/                    1533B  artifact=9   releaseId=r-0.2.108    Setup 없음
manifest-backups/0.2.109/                    1534B  artifact=9   releaseId=r-0.2.109    Setup 없음   ← 현장을 깨뜨린 그 문서
manifest-backups/0.2.109-20260910T151705Z/   1703B  artifact=10  releaseId=r-0.2.109-2  Setup 있음   ← 실제로 좋은 것
```
- 백업 가드가 덮어쓰기를 막은 것은 맞고 그것은 값을 했다. **그런데 결과적으로 `0.2.109` 라는 이름의 디렉터리가 고장난 manifest 를 들고 있다.**
- ⚠️ 지금 0.2.110 을 롤백해야 하면, **이름만 보고 고르는 사람은 장애를 일으킨 바로 그 문서를 복원한다.** 타임스탬프 쪽이 좋은 것이라는 사실은 **어디에도 적혀 있지 않다.**
- ⚠️ **더 넓다**: `0.2.108` 백업도 **artifact 9개, Setup 없음**이다. 즉 **맨 이름으로 된 백업은 전부 swap 을 끝낼 수 없는 문서**이고, 쓸 수 있는 롤백 지점은 **타임스탬프가 붙은 하나뿐**이다.
- ⚠️ #507 에 *"원본이 보존됐다"* 고 적은 것은 사실이지만, **"좋은 것이 보존됐다" 로 읽히면 틀린다.** 가드는 **덮어쓰지 않는 것**만 보장하고 **무엇이 좋은지는 모른다.**
- **업데이터 세대와 무관하다**(검증용 감사): 구 업데이터의 필수는 **9개**(정지 7 + ui 2, Setup 포함)라 맨 이름 둘은 **Setup 이 없어 실패**하고, 신 업데이터(0.2.110)의 필수는 **10개**(Updater 추가)라 역시 타임스탬프본만 성공한다. **어느 세대로 롤백하든 선택지는 하나다.**
- **진행형 결함은 아니다.** 다음 게시 때 백업되는 것은 0.2.110 pair(artifact 10)이므로 `0.2.110/` 이라는 맨 이름으로도 유효한 롤백 지점이 되고, **7번 게이트가 Setup 없는 manifest 를 애초에 못 나가게 막으므로 이 형태는 재발하지 않는다.** 과거 잔재다.
- **서버 파일은 손대지 않는다**(0.2.110 게시분과 무관, 급하지 않음). 방향은 계획 검토 후 결정 — 백업에 **releaseId 와 payload 검사 통과 여부를 표식으로** 남기거나 이름에 releaseId 를 넣는 쪽. 작업목록 **0.0.7**.

**⭐ 이번 라운드에 같은 형태가 세 번 나왔다 — 한 줄로 남겨 둔다**
> **검사하지 않고 통과를 보고한 것** — 셋 다.
1. 테스트의 `check` detail 이 **대기와 다른 시점**을 다시 읽어 `PASS … 1 left` 를 찍었다.
2. 배포 회귀의 **서명 변조가 변조를 하지 않았다**(`sed 's/^0/1/'` 인데 서명이 `0` 으로 시작할 때만 동작). 그 케이스는 **멀쩡한 릴리스를 게시하면서 "거부한다" 로 PASS** 했다.
3. dry-run 의 `verify_on_server` 가 **건너뛴 파일을 세지 않고** *"all 10 artifacts match"* 를 찍었다. **하나도 안 보고 10개가 맞다고** 한 것이다.
- 이것은 이번 사고의 **원인 축 3번(실패하는 함수를 stub 한 테스트를 실제 검증으로 승인)과 같은 계열**이다. 형태가 같다: **초록은 무언가가 검사됐다는 뜻이 아니다.**
- ⚠️ 셋 다 **코드가 아니라 보고 문구**에서 나왔다. 요약 줄은 **방금 한 일을 설명해야 한다** — 개수를 쓰려면 **검사한 개수**를, 상태를 쓰려면 **판정에 쓴 그 순간의 값**을.
...
- **정적 배선 검사**: `client_shell_main.cpp` 를 읽어 **호출 지점 2곳 이상 + 하나는 세션 생성 이후**임을 단정.
  ⚠️ **등급 정정**(검증용·Codex): 이것은 **정적 배선 검사**이고, *"실제 UI·인증 완료 → 팝업 → worker 동작"* 의 검증이 **아니다.** 한때 이것을 호출 사슬 검증처럼 취급했는데 등급을 올려 말한 것이다. **0.2.114 의 Client 실팝업은 미검증**이고, 사용자에게 *"이제 뜹니다"* 같은 보장 표현을 쓰지 않는다.
...
### 516) 2026-09-11 물러나는 Host 가 **자식을 확인하고 떠나게** — 그리고 내 주장 등급 4건 정정
계약은 사용자 문장 그대로다: **"host 만 살리고 자식 프로세스를 정리했어야지."**

**무엇이 문제였나**
인수인계 exit 경로(`kUpdateHandoffDoneMessage` 의 ExitNow 분기)는 **`DestroyWindow(window)` 만** 불렀고, 자식 정리는 `WM_DESTROY` 의 `g.streaming.Stop()` 에 맡겼다. 틀린 순서는 아니지만 **확인이 없었다.**
- 09-11 11:17 실측: ack(11:17:54.721) **뒤인 11:17:55** 에 생성된 `GNLinkStream`(pid 22204, 부모 22544)이 **부모보다 오래 살아남아 UDP 43000·3478 을 쥐었다.**
- 업데이터가 띄운 새 Host 는 `udp bind failed on every candidate port` → `code=3` → **`streak=36` 까지 재시도**했고 기기가 못 쓰게 됐다. **두 번** 그랬고, 복구는 사용자가 고아를 손으로 죽여서 됐다.
- ⚠️ **로그에 "자식을 남겼다" 는 말이 한 줄도 없었다.** 업데이터는 *"어떤 pid 에게 종료 요청을 못 했다"* 만 말했다. **인과가 로그에서 끊겨 있었다.**

**고친 것 (최소 수정)**
- `StreamingSupervisor::Stop(std::string* outDetail)` — **자식이 실제로 사라졌는지 확인하고 결과를 돌려준다.** 조기 반환 경로(`running_` 이 이미 false)도 **그냥 반환하지 않고** 자식 생존을 점검해, 살아 있으면 다시 요청하고 기한 대기한다.
- `ChildPid()` · `WaitForChildExit()` 추가. ⚠️ 핸들 대기가 아니라 플래그 대기다 — **핸들 소유는 `Supervise()` 단독**(Ledger H-01)이고, 이 경로는 `Supervise()` 가 없을 수도 있다.
- **인수인계 exit 이 `DestroyWindow` 전에 자식을 정리하고 기록한다**: `[host-app] update: standing down -- child pid N exited`. 실패하면 **다음 프로세스가 포트를 못 잡을 수 있다고 경고**한다.
- `WM_DESTROY` 도 결과를 기록한다 — *"already stopped, no child"* 와 *"아직 살아 있다"* 는 다른 답이고 **하나만 괜찮다.**
- ⚠️ **아키텍처 재설계 없음**(미승인 범위). `TerminateProcess` **광역 승격 없음**, 이름 기반 kill 없음, 업데이터가 남의 자식을 죽이는 방향 없음. 정리 대상은 **소유가 확인된 자기 자식뿐.**

**회귀**
`update_stop_process_test` **24 → 28 PASS**. 새 4건은 **정적 배선 검사**다: ExitNow 가 자식을 정지하고 · `DestroyWindow` **이전에** 하고 · 결과를 기록하고 · 남았을 때 경고한다.
⚠️ **등급을 분명히 한다**: 이 4건은 **소스를 읽는 검사**이고 *"실제 Host 가 물러날 때 그렇게 한다"* 를 증명하지 **않는다.** 그건 **실제 Host 를 돌려야만** 보이고 **실기 재시험의 몫**이다. 기존 실제 프로세스 반례(고아 (a)/(b) 양쪽·음성 대조)는 그대로 유효하다.

**🔴 내 주장 등급 정정 4건** (검증용·Codex 지적, 수용)
1. **"인앱 완주 0회" 총괄 철회.** 사용자는 Host 로 업데이트해 성공했다고 **명시 보고**했다. 그 보고는 **별도 사실로 보존**하고, 저희가 관측하지 못한 것(정확한 시도 시각·기기·트리거)은 **"미확인" 으로 분리**한다. 하나로 뭉쳐 "0회" 라고 쓰지 않는다.
2. **"로그 부재 = 실행 0" 은 확정이 아니다.** 무버퍼 로거라는 사실만으로는 부족하다 — 경로 차이·프로세스 수명·쓰기 실패가 남는다. **가능성**으로 낮춘다.
3. **"`result:` 부재 = `effects.run()` 미반환" 도 가능성**이다. 원인 확정이 아니다.
4. **https 를 전체 미검증으로 되돌리지 않는다** — 제품 WinHTTP 로 `/healthz` 성공 사례가 있다. 미검증 범위는 **Client 인증 경로와 실기**로 한정한다.

**🔴 사용자 안내 철회**: 회사 PC 에 **"Host 로 업데이트 1회"** 권유를 **철회했다.** 고아 결함이 그대로라 **회사 PC 에서도 같은 경합**이 날 수 있다. **안전한 bootstrap 방식이 확정되기 전에는 반복 설치도 권유하지 않는다.**
**0.2.114 게시분은 그대로 둔다**(임의 삭제·롤백 게시 금지). **추가 version-only 게시 금지** — 원인 근거와 좁은 수정이 먼저다.

### 517) 2026-09-11 `0.2.115` 게시 — **고아 수정이 실린 첫 릴리스**
`0.2.114` 는 Client 확인 수정만 실려 있었다. 고아 수정(`ecf340c`)이 들어간 릴리스는 이것이 처음이다.
⚠️ **그래서 손이 닿지 않는 기계에 깔 것은 `0.2.114` 가 아니라 이 릴리스다.** `0.2.114` 만 깔면 업데이트가
**보이게** 되고, 누르는 순간 고아 경합을 탄다 — 검증용 지적, 수용.

**선행 sweep** (`ecf340c` 검증분, build 14:04): **66 스위트 1865 PASS**, 실패 1건
`gdi_capture_process`(rc=1) — **알려진 문턱 스위트**(0.0.4: 문턱 20.0ms 가 관측 분산 16.8~23.3ms 안).
증분 **+4** 는 `update_stop_process_test` 에 넣은 `(static)` 검사 수와 일치한다.
⚠️ 총계는 **같은 빌드끼리만** 비교한다 — 오늘 두 번 다른 빌드를 맞대어 잘못 읽었다.

**산출물 대조** (0.2.114 → 0.2.115): 바뀐 것 **4개**뿐이다.
`GNLinkHost.exe` 660480→**664064** (고아 수정+버전) · `GNLinkClient.exe` · `GNLinkSetup.exe` ·
**`GNLinkUpdater.exe`** `17ecfa1e…`→`fd948c7f…`. 나머지 6개는 바이트 동일 — 안 바뀐 것이 재링크되지
않은 정상 결과다. 업데이터가 바뀐 것은 의미가 있다: **"업데이터가 자기를 교체했는가" 가 실기 관측점**인데,
바이트가 같으면 그 관측이 불가능했다.

**게시 전 게이트** (둘 다 통과)
- payload-set 게이트 — `product_payload_names()` 10개와 manifest 10개가 **이름까지 일치**
- `remote60_verify_release` — **제품에 컴파일된 키**로 실문서 `Ok`, **1바이트 변조 `SignatureInvalid`**.
  받아들이는 것만 보면 검증기가 아니다.

**게시** (`automation/gnlink_deploy.sh`, 순서 고정: preflight → 업로드 → **서버에서 검증** → 백업 →
manifest+sig **마지막에** 원자 교체 → 외부 https)
- manifest `9b4653e1…` 1687B · sig `7937fffe…`
- 서버 10/10 일치 · **공개 URL 10/10** 을 익명으로 내려받아 크기·sha256 재확인
- **서버가 내주는 문서를 다시 내려받아 제품 검증기로 재검증** — 서명된 바이트와 **완전 동일**
- 0.2.114 pair 백업됨(`be00ba15…`/`4203d4c4…` = 0.2.114 게시 시 값과 일치)
- **0.2.111~0.2.114 아티팩트 불변** 확인. `0.2.112` 디렉터리가 **비어 있는 것은 이전부터**이고 이번에
  건드린 것이 아니다

**아직 아닌 것**: 게시는 *"실기 통과"* 가 아니다. `still open  the field test: a real host taking
this update end to end` 는 스크립트가 스스로 찍는 줄이고, 그대로 유효하다. 실기 절차와 **복구 절차**는
`docs/update_field_retest_runbook.md` 에 먼저 적어 두었다 — 09-11 에 기기가 두 번 불능이 됐을 때
**무엇을 보고 무엇을 되돌리는지 적힌 곳이 없어서** 복구가 사람 손에 맡겨졌기 때문이다.

### 518) 2026-09-11 🔴 **알림은 배선됐고 행동은 배선되지 않았다** — Client 에 누를 것이 없었다
`0.2.114` 는 *"Client 가 업데이트를 물어본 적이 없다"* 를 고친 릴리스였다. 물어보기는 고쳐졌는데,
**답을 받은 뒤 할 수 있는 일이 없었다.**

**무엇이 문제였나** (Codex 발견, 검증용 확인, 저도 소스에서 재확인)
- `ui/shell.html` 의 `<button>` 전수: `signIn` `signOut` `refresh` `openSettings` `openLog`
  `closeSettings` `saveSettings` — **업데이트 버튼 0개**
- 페이지에서 `type:'update'` 를 보내는 곳 **0건**
- `client_shell_main.cpp:1037` 에 **수신 handler 는 있다.** 처음부터 있었다. **아무도 보내지 않았다.**
- 결과: 화면에 *"새 버전 0.2.115 이 있습니다."* 만 뜨고 **누를 것이 없다.**

**🔴 이건 이번 라운드 내내 경계한 바로 그 실패다.** *"코드는 있는데 아무도 안 부른다"* —
`log_upload_stop()` · `request_process_stop` · `GNLinkUpdater.exe` 의 `payloadNames` 와 같은 모양이다.
그런데 이번엔 **그것을 검사하겠다고 만든 테스트가 통과시켰다.** `client_update_flow_test` 는
`start_update_check` **호출 지점**을 셌고, **행동 경로는 보지 않았다.** 소스 grep 은 이 틈을 볼 수 없다 —
**양쪽이 다 있고, 없는 것은 둘을 잇는 사실뿐**이기 때문이다.

**고친 것**
- `shell_update_available_json` / `shell_update_cleared_json` / `shell_update_busy_json`
  (`client_shell_bridge.cpp`) — **버전이 필드로 간다.** 상태 줄의 한국어 문장을 파싱해 버전을 얻는
  페이지는 문구가 바뀌는 순간 깨진다.
- `shell.html` 에 **상단 배너 + `업데이트` · `나중에`**. `업데이트` → `send({type:'update'})` →
  기존 `:1037` → `start_client_update`. **`나중에` 는 연기일 뿐** — 로그인·목록·설정을 건드리지 않는다.
- **중복 기동 0**: 페이지가 버튼을 죽이고, **네이티브가 `gUpdateStarting` 으로 실제로 막는다.** 페이지에서만
  막으면 이미 날아간 두 번째 메시지를 못 막는다. 실패하면 **버튼을 돌려준다** — 죽은 채로 두면 알림만 있고
  할 수 있는 게 없는, 이 변경이 없애려던 바로 그 상태가 된다.
- **로그아웃은 제안을 회수한다** — 그 답은 물어본 세션의 것이고, 다음 사람에게 넘길 것이 아니다.
- ⚠️ **401·실패는 원래부터 표시되지 않는다** — `ShellUpdateNotice::show` 가 `UpdateAvailable` 에서만
  참이다. 이번에 바꾸지 않았고, 테스트로 확인했다.

**검증 — 소스를 읽지 않는다**
`client_update_ui_test` **17 PASS** (`remote60_client_update_ui_test`).
**실제 WebView2** 에 **배포되는 `ui/shell.html` 그 파일**을 띄우고, **제품 메시지 빌더**가 만든 문서를
넣고, **렌더된 요소를 DOM 으로 클릭**해서, **`{"type":"update"}` 가 실제로 나오는 것**을 확인한다.
스크린샷: `build-local/apps/native_poc/Release/client_update_ui.png` — 배너에 `새 버전 0.2.115 이
있습니다.` 와 두 버튼이 보인다.
양성/음성 모두: 제안 전 클릭 무응답 · 표시 · 버전 명시 · 클릭 송신 · busy 중 비활성 · 실패 후 복귀 ·
`나중에` 후 로그인 화면 불변 · cleared 회수 · 로그아웃 회수 · 회수 후 클릭 무응답.
회귀: `client_shell_bridge_test` 29 PASS · `client_update_flow_test` 23 PASS.

**부수 발견 2건**
- 테스트가 **빌드 디렉터리의 `ui/shell.html` 사본을 먼저 읽어 한 판 낡은 페이지를 검사할 뻔했다.**
  저장소 사본 우선으로 바꿨다. *"배포되는 그 파일"* 이 아니면 이 테스트는 의미가 없다.
- 첫 렌더의 문구가 깨졌다 — **제 테스트 타깃에 `/utf-8` 이 빠진 것**이고 제품은 `:1353` 에서 붙는다.
  **제품 결함이 아니다.** 타깃에 추가했다.
- WebView2 환경 생성이 `0x800401f0`(`CO_E_NOTINITIALIZED`) 로 실패했다 — 이번 세션 등록 경로와
  **같은 종류**다. 테스트에 아파트먼트를 붙였다.

**아직 아닌 것**: 실서버 확인 → 이 메시지 발생은 **미검증**이다. 계정과 네트워크가 필요하고 별도 단계다.
여기서 증명한 것은 **없던 절반** — 메시지가 도착했을 때 버튼이 있고, 누르면 네이티브가 기다리던 것이
나간다는 것뿐이다.

### 519) 2026-09-11 `0.2.116` 게시 — **누를 수 있는 첫 릴리스**, 그리고 http 예시 한 줄
`0.2.115` 는 고아 수정이 실린 첫 릴리스였지만 **Client 에 버튼이 없었다**(#518). 이것이 알림과 행동이
둘 다 있는 첫 릴리스다.

**같이 들어간 한 줄** (검증용이 #518 스크린샷에서 발견)
서버 주소 입력칸 placeholder 가 `http://주소:8080` 이었다. **업데이트 경로는 http 를 두 층에서 거부**하므로
이 예시는 사용자를 **업데이트가 조용히 꺼지는 설정으로 유도**한다 — 이 PC 의 저장값
`http://223.130.132.180:8080` 과 같은 형태이고, 그 값 때문에 이 PC 의 확인은 전부 `not configured` 였다.
`https://주소` 로 바꿨다. ⚠️ **그 이상은 하지 않았다** — 주소 검증도, 강제 승격도 범위 밖이다.

**산출물 대조** (0.2.115 → 0.2.116): 바뀐 것 **5개**.
Host · Client `711168`→**712192** · Setup · Updater · **`ui\shell.html` `12648`→`15483`**(배너·버튼·핸들러).
나머지 5개 바이트 동일.

**게시 전후** — `0.2.115` 와 같은 순서, 같은 게이트
- payload-set 게이트 통과 · `remote60_verify_release` **실문서 Ok + 1바이트 변조 `SignatureInvalid`**
- manifest `07ec0ad2…` 1687B · sig `435cf93d…`
- 서버 10/10 · **공개 URL 10/10** 익명 재확인 · **서버가 내주는 문서를 되받아 제품 검증기로 재검증**(서명 바이트와 동일)
- `0.2.115` pair 백업 · **`0.2.115` 아티팩트 4종 해시 서버에서 직접 대조 → 불변 확인**
- `dist/GNLinkSetup-0.2.116.exe` — **공개 URL 이 내주는 바이트와 `cmp` 로 동일 확인**(스테이징본 대조로 끝내지 않음)

**⚠️ 이 릴리스에 대해 말할 수 없는 것**
`client_update_ui_test` 17 PASS 는 **메시지가 도착했을 때 버튼이 있고 누르면 나간다**는 것까지다.
**실서버 확인 → 팝업**은 여전히 미검증이고, 계정·네트워크가 필요하다. 전달은 **(b) 수동 설치**로 한다 —
(a) UI 파일만 보내는 것은 **순환**이다: 버튼을 보내려면 버튼이 필요하고, 남은 경로인 Host 트레이는
0.2.114 Host 에 고아 수정이 없어 09-11 에 두 번 기기를 불능으로 만든 경합을 탄다.

**부기 — 관측 기준선을 잃었다**
14:37 이후 이 PC 의 Host·Stream 이 사라졌다. **원인 미상**이다. 제가 종료한 것은 관측용으로 띄웠던
임시 Client(pid 15640) 하나뿐이고, `host_app.log` 는 14:37:17 에서 끊기며 종료 줄이 없다.
**포트를 쥔 고아는 없다**(43000·3478 소유자 0) — 정상 종료와 모순되지 않는 모양이다. 단정하지 않고
기록만 한다. ④ 재시험은 어차피 `qwinsta` 가 **RDP Active** 라 판정 불가 상태였다.

### 520) 2026-09-11 UI 검사 보강 3건 — **부정 대조가 실제로 실패해야 검사다**
검증용·Codex 교차검토로 `client_update_ui_test` 의 실제 범위를 확정한 뒤의 보강이다. 그때까지
증명된 것은 *"배포될 HTML 이 그 메시지를 낼 수 있다"* 까지였다.

**1) `disabled` 가 아니라 "누를 수 있는가"**
`disabled === false` 는 부족하다 — 버튼은 **활성인 채로** 화면 밖·투명·다른 것에 덮여 있을 수 있다.
새 판정은 `offsetParent` · viewport 교차 · `getComputedStyle` 의 `visibility`/`opacity` ·
**`document.elementFromPoint(중심)` 이 그 버튼을 돌려주는지**(가림 검출) · **키보드 포커스 도달**을 본다.
**부정 대조 3건이 실제로 실패한다**: `display:none` → `not laid out` · 투명 오버레이 →
`covered by blocker` · 치운 뒤 → `pressable`. 셋을 **한 스크립트 안에서 원자적으로** 돌린다 —
왕복을 나누면 호출 사이에 페이지가 변한 채로 답이 섞인다.
**4번째 부정 대조**: **버튼이 없는 낡은 판**(0.2.115 의 페이지 모양)을 물리면 `no element` 로 실패하고
메시지도 못 보낸다. **이 파일이 0.2.115 를 잡았어야 했다는 것을 스스로 증명한다.**

**2) 검사 대상을 서명된 manifest 의 해시로 고정**
전에는 *"저장소 사본"* 을 읽을 뿐, **게시 산출물과 같다는 단정이 0건**이었다. 이제 **제품 `load_manifest()`
+ 컴파일된 운영 공개키**로 manifest 를 검증하고, 거기 적힌 `ui\shell.html` sha256 과 **로드할 파일의
해시가 다르면 FAIL** 한다. 검증되지 않은 manifest 는 무엇이 옳은지 말할 자격이 없다 — 그렇지 않으면
텍스트 파일을 고쳐 고정점을 옮길 수 있다.
⚠️ **같은 커밋·같은 툴체인이 같은 바이너리를 보장하지 않는다**(검증용 이전 주장 철회). 해시가 유일한 고정점이다.

**3) 증거 등급 3단 분리** — ⑴ 페이지가 WebMessage 를 보냈다 ⑵ 네이티브가 받아 행동했다 ⑶ 실제 업데이터가
떴다. **이 테스트는 ⑴까지**이고, ⑴을 ⑵⑶의 증거로 쓰지 않는다. 파일 머리에 못 박아 두었다.

**🔴 하네스 자체의 결함 2건 — 제품이 아니라 제 테스트 코드**
- **완료 콜백이 지역 변수를 참조 캡처했다.** `eval()` 과 `write_screenshot()` 둘 다. 호출이 제한 시간을
  넘기면 함수는 반환하고, **늦게 도착한 콜백이 사라진 프레임에 쓴다.** 증상은 실행마다 다른 지점에서 나는
  segfault 였고, **모든 충돌이 스크린샷 이후에 났다**는 것이 단서였다. `shared_ptr` 로 고쳤다.
- **한 표현식 안에서 `eval` 을 두 번 부르면 깨진다.** `ok(wait_for(...), "...", eval(...))` 형태가
  메시지 펌프를 한 인자 목록의 양쪽에서 재진입시킨다. `pump_until` 술어 안에서 `eval` 을 부르던 5곳도
  같은 이유로 걷어내고 `wait_for()` 로 평탄화했다.
- 결과: **4회 연속 23 PASS**(전엔 3회 중 2회 충돌). ⚠️ 한 번 통과한 것을 안정으로 읽지 않는다 —
  간헐 결함은 반복 실행으로만 보인다.

**회귀**: `client_shell_bridge_test` 29 PASS · `client_update_flow_test` 23 PASS.
**게시하지 않았다** — 사용자 설치가 먼저이고, `0.2.116` 게시분은 불변이다.

**🔴 여기서 나온 규칙 — "PASS 개수는 그 프로세스가 정상 종료했을 때만 결과다"**
이 라운드에서 제가 보고하고 검증용이 OK 를 낸 **"17 PASS"** 는 결과가 아니라 **중단 지점**이었다.
프로세스는 18번째 단정을 찍기 전에 죽었고, 그때까지 찍힌 PASS 만 화면에 남아 있었다. 종료 코드를 보지
않으면 *"여기까지는 통과했다"* 와 *"전부 통과했다"* 가 **똑같이 생겼다.**
- 스위트 결과는 **`^PASS` 개수 + 종료 코드 + 마지막 요약 줄**을 함께 읽는다. 셋 중 하나라도 없으면 결과가 아니다.
- 개수 비교는 **같은 구성끼리만** 한다. 이 파일의 17 과 23 은 항목 구성이 달라 **비교 대상이 아니다.**
- **간헐 결함은 1회 통과로 닫지 않는다.** 4회 연속으로 확인했다.

같은 계열의 사례가 오늘만 셋이다: `updater.log` 에 **`result:` 줄이 없는데 성공으로 읽은 것**(#512·#516),
**`^PASS` 를 한 줄도 찍지 않는 스위트 5개가 총계에 묻힌 것**(#509), 그리고 이것. 전부 *"보고가 끝까지
갔다는 증거를 따로 확인하지 않았다"* 는 한 가지 실수다.

### 521) 2026-09-11 정책 개정 — **전면 금지 3건 삭제, 기술 오기 2건 정정, 행동 기준 6절 신설**
`CLAUDE.md` 를 상세 정본으로 재작성(227줄)하고 `AGENTS.md` 는 상위만 남겼다(67줄).
삭제: **RDP 전면 테스트 금지** · **로컬 로그 무조건 금지** · **매 응답 장문 요약 강제**.
정정: **서버 재시작이 env 변경 때만 필요하다** 는 것 · **manifest+sig 가 pair 원자적으로 교체된다**
는 것 — 후자는 스크립트 자신이 `gnlink_deploy.sh:435-436` 에서 아니라고 적고 있었다.
신설: 사용자 행동 우선 완료 조건 · 테스트/출하 빌드 분리 · 사용자 손 최소화 · 연속 수행 ·
범위와 질문 · 검증 독립성과 재현.
전문과 근거는 `docs/history/history_2026-W37.md` 의 2026-09-11 정책 개정 절. (A2A task t-s9ggt7bw)

### 522) 2026-09-11 최우선 운영 수칙 — A2A 왕복 축소와 최종 보고 책임
- 목표: 사용자가 직접 요청한 정책 정비. 중복 지시·재촉·수동 대기와 Client 작업 최종 안내 누락을 방지한다.
- 변경: AGENTS.md 상위 수칙·문서 2단계 역할 예외, CLAUDE.md 통합 지시/revision/미결 조회 1개/단일 사용자 창구/기능 단위 독립 검사. 검증용은 문서 전용 편집·커밋 가능, Codex가 독립 검토한다. 제품 3역할·안전 경계·UAC 사용자 실기 분리는 유지한다.
- 기록: docs/history/history_2026-W37.md, docs/구현계획.md 운영 정책 상태 갱신.
- 검증: Codex가 직접 두 정책 문서를 수정하고 전문·상호 참조·역할 예외·잔존 충돌과 diff/공백을 확인. 문서만 변경하여 제품 빌드·테스트는 하지 않음.
- 다음: 지금부터 적용. 제품 코드·버전·dist·서버 변경 없음, Git push는 사용자 보류 유지. 이 완료는 기존 Client 실기 결과와 별개다.

### 522) 2026-09-11 `0.2.117` 게시 — **인앱 시험의 target 전용**
버전 상수만 올린 릴리스다. 기능 변경 0.

**산출물 대조**(0.2.116 → 0.2.117): 각인이 있는 **4개만** 바뀌었다 — Host `773db36f…` · Client
`9d0d3385…` · Setup `58696147…` · Updater `fc3ba552…`. **`ui\shell.html` 은 바이트 동일**
(`131bc7f5…`, 15,483B) 이고 나머지 5개도 동일하다. 버전 전용 릴리스는 이 모양이어야 한다.

**설치기 임베드 실 bytes 확인**: RT_RCDATA 9개 전부 빌드 산출물과 일치(`verify_installer.ps1`).
그중 `shell.html id=206 15,483B 131bc7f5…` — **버튼이 들어간 판이 설치기 안에 있다.** 이름 검색이
아니라 리소스 바이트를 읽어 해시한 결과다.

**게이트·게시**: payload-set 10/10 · 제품 `default_verifier` **실문서 Ok + 1바이트 변조
`SignatureInvalid`** · manifest `f5221afb…` / sig `b16a865f…` · 서버 10/10 · **공개 URL 10/10**
익명 재확인 · **서버가 내주는 pair 를 되받아 제품 검증기로 재검증**(서명 바이트와 완전 동일) ·
**0.2.116 pair 백업** · **0.2.116 아티팩트 3종 서버 직접 대조 → 불변**.

**🔴 전제가 아직 안 맞는다 — 게시만으로 시험할 수 없다**
이 PC 설치본은 **`DisplayVersion=0.2.114`**, 설치된 `ui/shell.html` 은 **12,648B · `updateNow` 0건**
= **버튼이 없는 판**이다(검증용 실측). 그래서 순서는 **① `0.2.116` 수동 설치(버튼 들어간 빌드,
UAC 1회) → ② 그 116 에서 버튼을 눌러 117 로 인앱 시험** 이다.
⚠️ **`0.2.117` 설치본을 수동 설치하면 안 된다** — 그러면 설치 버전이 곧 최신이 되어 **인앱 시험
대상이 사라진다.** 그래서 이번에는 **`dist/GNLinkSetup-0.2.117.exe` 를 일부러 만들지 않았다.**

**미검증**: 인앱 완주 · Client 실팝업(실서버) · Host 고아 수정 실거동.

### 523) 2026-09-11 `0.2.118` — **배너가 늘 보이던 CSS 특이성**, 그리고 **Host 한글이 깨진 빌드 옵션**
실기에서 나온 두 건. 원인은 둘 다 확정됐고 바이트로 전후를 대조했다.

**A. 로그인 전에도 배너가 보였다 — CSS 특이성**
`.hidden { display: none }` 는 **클래스(0-0-1-0)**, `#updateBar { display: flex }` 는 **ID(0-1-0-0)**.
**ID 가 이겨서 `hidden` 클래스를 붙여도 안 숨었다.** 스크립트는 정확히 의도대로 클래스를 붙이고 있었고,
화면만 그 말을 듣지 않았다. 수정은 한 줄 — `#updateBar.hidden { display: none }`.

🔴 **테스트가 이걸 통과시킨 이유가 더 중요하다.** 숨김을 `classList.contains('hidden')` 로 검사했는데
그 값은 **버그가 사는 내내 참이었다.** 클래스를 읽는 것은 **의도**를 읽는 것이고, 필요한 것은 **결과**였다.
이제 `getComputedStyle().display` + rect + **hit-test** 로 판정한다.
**음성 대조 신설**: 런타임에 `#updateBar.hidden` 규칙을 **지우면** `"1:flex"` — 즉 **옛 CSS 를 되돌리면
배너가 실제로 다시 보인다.** 이 대조가 통과하지 못하면 수정이 사라진 것이다. `client_update_ui_test`
**23 → 25 PASS**.

**B. Host 한글 깨짐 — `/utf-8` 이 그 타깃에만 없었다**
`remote60_client_shell` 과 테스트 타깃 십여 개에는 있는데 **`remote60_host_app` 에는 0건**이었다.
소스는 UTF-8 인데 MSVC 가 시스템 코드 페이지(949)로 읽어 **잘못 읽은 문자를 그대로 저장**했다 —
`host_app_main.cpp` 의 **C4819 경고가 오래전부터 그렇게 말하고 있었다.**
**바이트 전후 대조**(추측 아님):
| 문자열 | 수정 전 | 수정 후 |
|---|---|---|
| `새` (UTF-16 `c8c0`) | **없음** | **485208** |
| `업데이트` | **없음** | **485130** |
| `지금 업데이트하시겠습니까?` | **없음** | **485124** |
그 타깃의 **C4819 = 0**. ⚠️ **영문으로 바꿔 숨기거나 깨진 리터럴을 고정하지 않았다** — 소스 문구는
그대로 두고 **읽는 방식**을 고쳤다.

**릴리스** `0.2.118`: 변경 5(Host `f3d9e537…` · Client `e67e3813…` · Setup `0a4e91f4…` ·
Updater `05ed59a5…` · **`ui\shell.html` 15483→15867 `8a081f96…`**), 동일 5.
설치기 RCDATA **9/9 실 bytes 일치** · payload-set 10/10 · 제품 검증기 **Ok + 1바이트 변조 거부** ·
manifest `e0d17d3a…` · 서버 10/10 · **공개 URL 10/10** · **서버 pair 되받아 재검증(서명 바이트 동일)** ·
`0.2.117` pair 백업.

⚠️ **`dist/GNLinkSetup-0.2.118.exe` 를 두지 않는다**(0.2.117 과 같은 이유) — 최신본을 수동 설치하면
**인앱 시험 대상이 사라진다.** 사용자가 설치할 것은 **`dist/GNLinkSetup-0.2.116.exe`** 이고,
...
**C1. 물러나는 중인 Host 를 "요청 실패" 로 읽었다** (로그에 남은 4회 중 3회)
⚠️ 로그가 직접 증명하는 것은 **stop 요청이 실패했다**는 것까지다. *"부모/자식 상태 매핑이 실패했다"* 는
**추적 가설**이고 원인으로 확정하지 않는다. 로그가 없는 시도가 있을 수 있으므로 **"전부 실패" 로
일괄 단정하지 않고** 확인된 attempt 의 시각과 결과만 적는다.
```
...
### 525) 2026-09-11 🔴 **도구 거부를 결과로 읽었다 — 파일은 이미 복사돼 있었다**
`dist/GNLinkSetup-0.2.118.exe` 를 복원하려다 도구 실행이 거부됐다. 나는 **"복원하지 못했습니다"** 라고
보고했다. **틀렸다.** 검증용이 실측했고 나도 확인했다:
```
dist/GNLinkSetup-0.2.118.exe  4,379,136B  0a4e91f4748fc785…  mtime 17:10:45
.claude/rel/0.2.118/payload/GNLinkSetup.exe  동일 (cmp 일치)
```
**복사는 실행됐고**, 거부된 것은 그 뒤에 이어진 부분이었다. 나는 **파일을 보지 않고 중간 신호만으로
결과를 단정했다.**

그리고 그 거부를 **사용자 의사로 해석**해 "사용자가 막았다" 고 적었다. 사용자 확인은 *"내가 안 막았는데"*
였다 — 실제로는 **하니스의 자동 승인 분류기**였다. 두 가지를 구분해 보고해야 한다: **사용자 거부**와
**분류기 거부**는 다른 사건이고, 후자는 명령 형태를 바꿔 재시도할 수 있다.

**오늘 같은 모양이 네 번째다.** `result:` 줄이 없는데 성공으로 읽음(#512·#516) · `^PASS` 를 안 찍는
스위트가 총계에 묻힘(#509) · **"17 PASS" 가 실은 중단 지점**(#520) · 그리고 이것. 전부
**"보고가 끝까지 갔는지, 결과가 실제로 그러한지를 따로 확인하지 않았다"** 는 하나의 실수다.
규칙(#520)을 한 줄 넓힌다: **종료 코드·요약 줄만이 아니라, 파일을 바꾸는 명령은 바뀐 파일을 본다.**

### 526) 2026-09-11 `0.2.120` 게시 — **고친 업데이터가 처음으로 실행 주체가 된 뒤의 시험 대상**
17:26 실기가 또 실패했다. ⚠️ **그 실패는 C1·C2 수정의 반증이 아니다** — 그때 업데이트를 실행한 것은
**설치본에 있던 `0.2.116` 의 업데이터**(`4f2b3a68…`)였고, 수정은 `0.2.119` 의 업데이터
(`9f337637…`) 안에 있었다. **고친 코드가 실행된 적이 없다.**
그리고 그것을 받으려면 업데이트가 성공해야 하는데 옛 업데이터가 실패시킨다 — **순환**이었다.
*"버튼만 누르면 된다"* 는 안내가 그래서 틀렸다.

검증용 실측으로 확인된 그때 상태: `GNLinkStream` 24844(부모 26280 부재, 16:49:37 생성)가 **38분째
UDP 43000 을 쥐고 있었다** — 16:49 실패가 남긴 고아다.

**순환을 끊은 것은 설치본 수동 설치다.** 사용자가 `0.2.119` 를 설치했고, 그 뒤 설치
`GNLinkUpdater.exe` = **`9f337637…`**, `DisplayVersion=0.2.119`, **GNLink 프로세스 0 · UDP
43000·3478 해제**(고아도 함께 정리). **이제 처음으로 C1·C2 가 든 업데이터가 실행 주체다.**

그래서 올라갈 대상이 없어져 `0.2.120` 을 버전 전용으로 냈다. 변경 4(각인 있는 Host·Client·Setup·
Updater), **`ui\shell.html` 포함 6개 동일**. 설치기 RCDATA 9/9 · payload-set 10/10 · 제품 검증기
**Ok + 변조 거부** · manifest `a8d9e35f…` · 서버 10/10 · **공개 URL 10/10** · **서버 pair 되받아
재검증** · `0.2.119` pair 백업 · **`0.2.119` 아티팩트 서버 직접 대조 → 불변**.
⚠️ **`dist/GNLinkSetup-0.2.120.exe` 를 만들지 않았다** — 이번엔 120 이 **시험 대상**이라 수동 설치되면
안 된다. `dist` 의 119 는 복구용으로 남긴다.

**미검증**: 인앱 완주. 다만 이번에는 **처음으로 수정된 업데이터가 그것을 수행한다** — 지금까지의 실패와
조건이 다르다. 그렇다고 성공을 예단하지 않는다.

### 527) 2026-09-11 UI 1차 — **preview 가 잡은 결함 2건**, 그리고 문구가 로그였던 자리
`docs/ui_state_table.md`(상태표)와 `remote60_ui_preview`(배포 `ui/shell.html` 을 실제 WebView2 에
렌더해 상태별 PNG 30장)를 먼저 만들었다. **도구는 판정하지 않는다** — 단정은
`client_update_ui_test` 가 한다. 증거를 만드는 것과 판정하는 것을 한 도구에 두면 증거가 아니다.

**P1. 좁은 창에서 배너가 내용을 덮었다** (`w360-s4-update-*.png`)
배너는 `position: fixed` 인데 본문에 여백이 없었다. 520px 에선 2줄이라 안 겹치지만 **360px 에선
4줄이 되어 `GNLink` 로고를 완전히 가렸고** `나중에` 가 잘렸다. 고친 뒤 다시 찍었더니 이번엔 **버튼
글자가 버튼 안에서 줄바꿈**됐다 — 한 번의 수정으로 끝나지 않았고, **두 번째 사진이 그것을 보여줬다.**
최종: 본문을 **실제 배너 높이**만큼 밀고(고정 여백이 아니라 `offsetHeight`, resize 에도 따라감),
배너는 `flex-wrap` 으로 **버튼을 아랫줄로** 내린다.

**P2. 연결 중인 카드가 "연결 가능" 이라고 말했다** (`*-s2-hosts-connecting.png`)
정보가 없던 게 아니라 **틀린 상태를 말하고 있었다**(검증용 정정). 상태 줄에만 `… 창을 여는 중` 이
나오고 카드는 초록 점 그대로였다. 이제 그 카드가 **파란 테두리 + 스피너 + `연결하는 중…`** 이고 끝나면
표시가 사라진다 — **그 사라짐이 정상 복귀의 관측점**이다.
⚠️ 첫 촬영은 **상태 메시지를 주입**해 찍었는데, 그 표시는 **페이지의 클릭 핸들러가 붙인다.** 메시지로
연출한 사진은 *"아무도 누르지 않았을 때의 화면"* 이었다 — 즉 **틀렸던 그 화면**이다. preview 가
**실제 클릭**하도록 고쳤다.

**문구 — `window_list_received count=3` 이 사용자 화면에 있었다**
`status` 한 필드가 **두 가지 일**을 했다: `should_append_selected_title()` 가 **접두사로 매칭**하는
토큰이면서, picker 가 화면에 그리는 문장이었다. 토큰은 계약이라 건드리지 않고 `displayStatus` 를 나눴다.
⚠️ 그리고 **한글 문장을 공유 코어에 넣었다가 빌드가 깨졌다** — 그 파일은 스무 개 넘는 타깃이 쓰는데
대부분 `/utf-8` 이 없어 **CP949 로 읽히며 닫는 따옴표를 잃는다**(`C2001`). Host 의 깨진 문구와 **같은
원인이 컴파일 단계에서 나타난 것**이다. 문장은 **그리는 곳**(viewer)으로 옮겼다.
바뀐 문구: 창 제목 `remote60 native video client` → **`GNLink`** · picker 헤더 `Remote60` →
**`공유 화면 선택`** · `Refresh`/`Desktop` → **`새로 고침`/`전체 화면`** · 진행 문구 3종 한글화 ·
잠금 문구 한글화. 바이너리 바이트로 확인(옛 문자열 부재, 새 문자열 존재).
덤: 뷰어 타깃의 `/utf-8` 이 **WebView2 조건부 안에** 있었다 — 사용자 문구의 인코딩이 **무관한 SDK 발견
여부에 달려 있었다.** 밖으로 뺐다.

**🔴 미루는 것 하나 — Desktop 카드 중복 제거**
우상단 버튼과 `Desktop (full screen)` 카드가 **둘 다** 전체 화면 선택이다. 제거하려면 `layout.
desktopButtonRect` 의 **레이아웃·히트테스트**를 고쳐야 하는데, 그 영역이 지금 **원인 미확정인 "대상
선택 무반응"** 과 겹친다. **진단 대상을 바꾸면서 진단할 수 없으므로** 원인을 좁힌 뒤로 미룬다.
...
### 528) 2026-09-12 `0.2.121` 게시 — UI 개선 묶음, **그리고 사진이 세 번 거짓말했다**
`gnlink-ui-refresh` v2/v2.1 의 결과물. 이번 회차에서 **제일 값이 나간 도구는 스크린샷**이었고,
**제일 자주 틀린 것도 스크린샷**이었다.

**제품 결함으로 고친 것**
- 🔴 **오류 문구가 화면에 오지 않았다.** 제품이 넣는 상태는 `control_disconnected` ·
  `session_lost` · `control_connect_failed` 같은 **개발 토큰**이다. 전에는 그대로 그려졌고
  (`window_list_received count=3` 과 같은 종류), `displayStatus` 를 분리한 뒤에는 **아무것도 채우지
  않아** 목록 유도 줄이 나왔다 — **개발 문자열 노출을 틀린 정보 표시로 바꾼 셈**이다. 토큰마다 한국어
  문장을 붙이고, **모르는 토큰은 그리지 않는다.**
- 🔴 **`if (!connected) statusLine = "호스트에 연결하는 중…";` 가 무조건 마지막에** 실행돼 방금 기록한
  실패 사유를 덮었다. 게다가 끊긴 뒤에는 **아무것도 연결하고 있지 않다** — 거짓 진행 표시였다.
  그 문구는 이제 **대기 토큰이 제어 채널 미연결일 때**만 나온다.
- **푸터가 무엇을 재는지 말한다**: `연결됨` → **`제어 연결됨`**. `session_lost` 는 워치독이
  `connected` 를 건드리지 않아 **제어는 살고 미디어만 죽은** 실제 상태다. 값을 억지로 맞췄다면
  **제어가 살아 있는데 끊겼다고 말하는 또 다른 거짓 표시**가 됐다.
- **시작 프로그램 이름** `remote60` → `GNLink`. ⚠️ **개명만 했으면 끌 수 없는 자동 시작**이 됐다 —
  옛 항목이 계속 부팅 실행되는데 앱은 꺼졌다고 보고하고 "끄기" 는 새 이름만 지운다. **두 이름을 함께**
  다룬다(읽기 OR · 켜기 새것+옛것 삭제 · 끄기 둘 다).
- **Desktop 중복 제거** — 버튼과 카드가 **같은 `begin_pc_target_selection(ctx, 0, …)`** 을 불렀다.
- **강조색 통일**(picker 초록 → 제품 파랑), 선택은 **테두리+틴트+✓**(색만으로 구분하지 않는다).
- **카드 격자가 항목 수를 모른다** → 5장이 화면의 1/5 만 쓰던 것. **키우기만 하고 줄이지 않는다.**
- 배너가 좁은 창에서 **로고를 덮던 것**, 연결 중 카드가 **"연결 가능" 이라고 말하던 것**, 슬라이더
  값이 **`restore` 전에는 빈 칸**이던 것, `새로고침`/`새로 고침` 불일치.

**🔴 스크린샷이 제품이 만들 수 없는 상태를 찍은 것이 세 번**
설정 화면을 `restore` 없이 · 연결 중 카드를 상태 주입으로 연출 · 끊김 토큰을 `connected=true` 인 채.
셋 다 **아무도 도달하지 않는 화면**을 증거로 남길 뻔했다. 각 shot 이 **제품과 같은 순서·플래그**를
쓰게 고쳤다. **증거 생성기가 제품 순서를 따르는지**가 기본 점검 항목이다.

**"대상 선택" 조사** — `picker_open_chain_test` **11 PASS**: 진짜 마우스 메시지 → 실제 툴바 →
제품 함수 → picker 가 **클라이언트 영역 100% 를 칠함**. **로그가 아니라 픽셀로** 판정했다.
가설 **1a·1c·2·3 은 결함이 아니고**, 남은 후보는 **1b·4·5**. ⚠️ 가설 5(reveal 경합)는 이 재현에
**swapchain 이 없어** 만들 수 없다. 원인 미확정이라 **수정하지 않았다.**
기대값을 두 번 틀렸고 **두 번 다 코드가 아니라 내가 틀렸다**(4% · 93%). 출력에 맞춰 덮었으면 묻혔다.

**게시**: manifest `88d76648…` / sig `f8b65c96…` · 변경 6(Host·Client·**Viewer**·Setup·Updater·
`ui\shell.html` 15867→19028) · 설치기 RCDATA 9/9 · 게이트 2개(**Ok + 변조 거부**) · 서버 10/10 ·
**공개 URL 10/10 전체 해시** · 서버 pair 되받아 재검증 · **0.2.120 pair 백업 + 아티팩트 불변 확인** ·
부분 pair 실패 없음. `dist/GNLinkSetup-0.2.121.exe` = **공개 URL 바이트와 `cmp` 동일**.
**해시 pin 이 게시 후 해소됐다** — `client_update_ui_test` **27 PASS rc=0**(게시 전에는 FAIL).

**미검증**: 인앱 완주 · "대상 선택" 실기(1b·4·5) · 키보드 포커스 링의 실제 렌더 · 실제 고DPI 모니터.

### 529) 2026-09-12 Codex NEEDS_CHANGES 3건 — **완료를 철회하고 고쳤다**
`0.2.121` 을 완료로 닫은 뒤 Codex 검토에서 결함 2건 + 내 문서 과잉 제한 1건이 나왔다.
⚠️ **게시된 `0.2.121` 에는 아래 수정이 들어 있지 않다.** 다음 릴리스에 실린다.

**⑴ 빈 목록 문구가 상태를 보지 않았다** (`viewer_overlay_draw.cpp:262`)
`windowItems.empty()` 만 보고 **어떤 이유로 비었든 같은 말**을 그렸다. 연결 실패 화면 실측:
헤더 `호스트에 연결하지 못했습니다.` / 중앙 `창이 없습니다. 새로 고침을 눌러 보세요.` /
`새로 고침` **비활성**. **상충하는 두 안내 + 누를 수 없는 행동 권유.**
"비었다" 는 그 자체로 뜻이 없고 **뜻은 상태가 정한다.** 헤더가 이미 사유를 말하는 상태에서는 중앙을
비우고, **연결돼 있는데 공유할 창이 없을 때만** 기존 안내를 낸다.
⚠️ 나는 같은 패턴을 `picker-error-disconnected.png` 에서 **보고도** 푸터만 고치고 중앙을 넘겼다.

**⑵ 자동 시작 마이그레이션이 쓰기 실패를 확인하지 않았다** (`host_app_main.cpp:797`)
`RegSetValueExW` 반환값을 버리고 **무조건** legacy 를 지웠다. 쓰기가 실패하면 **정상 동작하던 항목만
사라지고 대체가 없다** — UI 는 "켜짐" 인데 부팅 때 아무것도 뜨지 않는다.
**성공했을 때만** 옮기고, 실패하면 기존 항목을 **그대로 두고 로그에 남긴다.**
🔴 이 함수는 *"개명만 하면 끌 수 없는 자동 시작이 된다"* 를 피하려고 쓴 것인데, **그 수정 안에 같은
종류의 구멍**을 남겼다. 한 함정을 피하면서 옆에 하나 더 판 셈이다.

**⑶ 내 문서가 지시보다 좁았다** — `ui_state_table.md` §9 의 *"fixture 는 와이어를 타므로 금지"* 는
**과잉 제한**이었다. 금지는 **운영 세션 접촉·프로토콜 변경**이지 **격리 실행 자체가 아니다.** 정정했다.
→ 대상 선택 **1b·4 는 자동 검증으로 닫을 수 있고**, **5 는 swapchain 이 있는 격리 제품 경로** 준비
가능성이 조사 대상이 된다. 내가 "불가" 로 닫아 둔 길이 실은 열려 있었다.

**⑷ 표현 정정** — *"UI 전면 개선 완료"* 로 말하지 않는다. **고친 것**(오류 문구·연결 중 표시·강조색·
격자·중복 제거·명칭·배너 겹침·슬라이더 값·포커스 링 정의)과 **확인 후 그대로 둔 것**(썸네일 종횡비·
auto-hide·목록/설정 디자인)은 다르다. 후자는 개선이 아니라 **결함 없음 확인**이다.
...
### 2026-09-12 장애 수정안·출시 준비 문서 초안 (다중 세션 검토 대기)

- 목표: 보고된 멈춤·드래그·입력 지연·불규칙한 프레임·화질 변화의 수정 방향과 출시 전 필요 작업 정리.
- 파일: `docs/release_readiness_2026-09-12.md` 신규, `docs/incident_2026-09-12_code_review.md` 연결, `docs/구현계획.md` 상태 및 본 이력.
- 결과: R01~R09별 수정 후보/보존 조건/검증 증거, 표시 간격 분포·입력 종단 반응·장시간/동시 세션 시험 계획. 현재 reconnect soak의 종료 코드 위주 판정, WAN gate의 필수 지표 누락 시 통과 경로를 코드로 확인해 보강 대상으로 기록.
- 검증: 관련 소스·기존 원장·자동화 스크립트 대조, 문서 diff 공백 검사. 제품 수정/테스트 실행/설치/배포 없음. 수치 목표는 신규 제안이며 기존 SLA 또는 측정 통과 결과가 아님.
- 다음: 사용자의 A2A 상대 확인 절차에 따라 검증용claude/작업용claude 구성 선택을 기다림. 선택 전 위임·독립 검토 미수행이며 다중 세션 조사 완료로 처리하지 않음.
- Git MCP 도구 미제공으로 커밋 없음. push 미실행. 다른 세션의 제품 릴리스 변경은 이번 작업과 구분.

### 534) 2026-09-12 🔴 **낡은 표찰은 표찰이 없는 것보다 나쁘다** (`ca7997b` 기록, #530 후속)
가설 5 재현이 들어 있는 파일이 이렇게 시작하고 있었다:
```
// Hypothesis 4: the input gate.
...
// The video present path being absent is also why this harness cannot speak to
// hypothesis 5: there is no swapchain here.
```
**present 를 실제로 링크하기 전에 쓴 문장이고, 같은 커밋 안에서 사실이 아니게 됐는데 갱신하지 않았다.**
CMake 주석도 `# Hypothesis 4:` 로 남은 채 바로 아래 줄에서 `d3d11 … mfplat` 을 링크하고 있었다.

결과: **검증용이 재현을 두 번 요청했다.** 재현은 커밋돼 있었고, 검증용은 그 타깃을 **이미 14 PASS 로
실행까지 해 놓은 상태였다.** 그런데 파일 머리말이 "여기서는 할 수 없다" 고 말하고 있었으니, 요약만
보고 본문을 안 읽었다면 그 문장이 그대로 답이 된다.

**증거가 멀쩡해도 표찰이 반대면 증거로 쓰이지 않는다.** 없는 표찰은 읽는 사람을 본문으로 보내지만,
틀린 표찰은 **본문을 안 읽어도 된다고 설득한다.** 그래서 없는 것보다 나쁘다.

같은 형태가 이 회차에만 네 번 있었다 — ⑴ picker 오류 화면을 만들어 놓고 **중앙 문구를 안 읽음**
⑵ 이 머리말 ⑶ `release_swapchain()` 이 *"GDI 로 돌려준다"* 고 적힌 주석 두 곳 ⑷ `viewer_startup.cpp`
의 *"its repaint is composited"* — **고쳐지기 전에 고쳐졌다고** 적혀 있었다(#532).
⑶⑷ 는 서로를 뒷받침해서 **틀린 전제가 세 곳에서 확인되는 것처럼** 보였다.
...
### 2026-09-14 Native 커밋 분리 02 — pipe/진단 종료 및 HTTP 오류 경계

- 목표: 백업 `backup/host-pc-recovery-daacbb2`의 변경을 의존성 순서로 재구성. 새 기능 추가나 배포가 아니다.
- 변경: bounded_process_exit.hpp, bounded_pipe_io.hpp, secure_input_broker.cpp, update_credential_channel.cpp, winhttp_transport.cpp, winhttp_recovery_test.cpp; 필요한 공유 파일 부분 변경과 CMake.
- 검증: 해당 단계 소스에서 Release 빌드(remote60_native_video_host_poc, remote60_update_credential_channel_test, remote60_winhttp_recovery_test) 및 관련 실행 2개 종료0. 증거 `.claude/split-02-results.json`. 기존 전체 실기 미검증과 release manifest pin 미완료는 유지.
- 다음: 다음 의존 단계 구성·검증. 메인 머지·push·배포 없음.


### 2026-09-14 Native 커밋 분리 03 — UDP control 총량·TTL·조각 검증

- 목표: 백업 `backup/host-pc-recovery-daacbb2`의 변경을 의존성 순서로 재구성. 새 기능 추가나 배포가 아니다.
- 변경: udp_control_channel.cpp, udp_control_channel.hpp, udp_control_channel_test.cpp; 필요한 공유 파일 부분 변경과 CMake.
- 검증: 해당 단계 소스에서 Release 빌드(remote60_udp_control_channel_test) 및 관련 실행 1개 종료0. 증거 `.claude/split-03-results.json`. 기존 전체 실기 미검증과 release manifest pin 미완료는 유지.
- 다음: 다음 의존 단계 구성·검증. 메인 머지·push·배포 없음.


### 2026-09-14 Native 커밋 분리 04 — Viewer 입력 큐와 영상 밖 버튼 해제
...
### 2026-09-14 Native 커밋 분리 06 — Host 초기화·종료·캡처 수명/대상 보존

- 목표: 백업 `backup/host-pc-recovery-daacbb2`의 변경을 의존성 순서로 재구성. 새 기능 추가나 배포가 아니다.
- 변경: capture_callback_gate.hpp, host_capture_device.cpp, host_capture_session.cpp, host_capture_session.hpp, host_control_session.cpp, host_input_inject.cpp, host_input_inject.hpp, host_loop_helpers.cpp, host_session.hpp, host_shutdown.cpp, host_startup.hpp, host_startup_capture.cpp, host_startup_connect.cpp, host_startup_graphics.cpp, host_watchdog.hpp, native_video_host_main.cpp, host_runtime_failure_test.cpp; 필요한 공유 파일 부분 변경과 CMake.
- 검증: 해당 단계 소스에서 Release 빌드(remote60_native_video_host_poc, remote60_host_runtime_failure_test) 및 관련 실행 1개 종료0. 증거 `.claude/split-06-results.json`. 기존 전체 실기 미검증과 release manifest pin 미완료는 유지.
- 다음: 다음 의존 단계 구성·검증. 메인 머지·push·배포 없음.


### 2026-09-14 Native 커밋 분리 07 — encoder pending·출력 복구와 tune epoch
...
### 2026-09-14 Native 커밋 분리 10 — Host/Client 인증·로그·업데이트 작업 수명
...
### 2026-09-15 집 Host 재접속 실패 조사 — 제어 디스패처 무기한 차단 (0.2.126, 조사 전용)

- 목표/승인: 사용자 긴급 지시. 13:45 고화질 테스트·AMD 오류 후 Host 접속 실패, 재시작 2회에도 지속. 로그·이벤트·소스 대조만, 제품 수정·배포·앱 조작 없음. 정본 `.claude/urgent-126-20260915/incident.md`(rev12), 요약 `docs/incident_2026-09-15_host_control_dispatcher.md`.
- 확정: 13:47:19~31 OS GPU 워치독 라이브 덤프 7건과 같은 창에서 Stream DXGI 획득 타임아웃 → 제품 워치독 code=44 자진 종료·재기동. 그 뒤 세 프로세스 모두 첫 세션 이후 제어 디스패처(프로세스당 1개)가 세션을 끝내지 못함(사고 전·2일치 회전본은 전부 정상). 감독자는 자식 종료만 보므로 막힌 디스패처는 감시 밖. AwaitControlReady 1500ms 상한 때문에 뷰어에는 연결됨으로 보임. 15:52:29 같은 프로세스에서 1h50m 만에 Serve 반환·즉시 정상, 그 전후 창 10→9(LDPlayer 소멸, 전후 열거 추론).
...
### 2026-09-15 응답 없는 창 썸네일 건너뛰기 — 0.2.128 동결과 그 사이 고친 것들

- 동결: 릴리스 source commit **`9559368`**(`product_version.hpp` 0.2.127→0.2.128, 1파일 1줄, BOM·CRLF 보존). 증분 빌드 산출물에서 `GNLinkStream` UTF-16 `0.2.128`=1 / stale `0.2.127`=0 확인. ⚠️ **clean build 와 게시 전 payload stale 재확인은 검증용 몫**으로 남겼다 — 0.2.126 의 REL-V01 이 증분 stale object 산물이었기 때문이다.
- **검증용이 찾은 제품 결함(D1, `c455b78`)**: `thumb_busy`·`thumb_helper_lingering` 이 `Failed` 로 나가 제어 세션이 **창 예산을 소모**했다. 둘 다 호스트 자원 경합이지 창 잘못이 아니고, 디스패처가 둘이라 3회는 금방 찬다 — **아무 잘못 없는 창이 60초 조용해지는**, 이 task 가 막으려던 증상이 다른 길로 돌아온 것이다. `Canceled`(무비용)로 바꿨다. 회귀는 두 각도에서 본다: 거부가 `Canceled` 인가(race 검사), 거부 3회 뒤에도 창이 `Allow` 인가(`cb49629`).
- **lingering 분기 실행(`4106adb`+`cba005d`)**: `TerminateProcess` 는 우리가 띄운 프로세스에 실패하지 않으므로 그 분기는 도달 불가였고, 지워도 깨지는 시험이 없었다. kill 확인을 **별 TU 로 분리**해 시험만 실패 구현을 링크한다 — 런타임 스위치가 아니라 **링크 타임**이라 출하물엔 경로 자체가 없다. 실행 결과: 다음 요청이 **3µs 에 거부**, 두 번째 helper 미기동, 회수까지 13/13.
- **V9 3구간 분해(`df07b76`)**: deadline 이 묶는 것은 **wait** 이고 teardown 은 `kKillConfirmMs` 상한의 **별도 예산**이다. 실측 — Ok: spawn 41.5~44.6 / wait 48.8~58.2 / teardown 3.7~4.3ms. TimedOut(400ms): spawn 139.9~140.9 / wait 398.9~399.6 / teardown 1.6~1.7ms. 주입: spawn 104.9 / wait 400.6 / teardown 3µs. ⚠️ 내가 앞서 보고한 "kill 확인 ~166ms" 는 **측정이 아니라 산술**이었고 틀렸다.
- **내 시험이 사용자 기계를 건드린 경로 둘을 닫았다.** ⑴ 스크래치 Stream 이 `--bind-address` 기본값 `INADDR_ANY` 로 **와일드카드 바인드**해 방화벽 프롬프트를 띄웠고 무응답분이 **Block 규칙 6개**로 남았다 → 루프백 바인드(`3284169`) + 호스트 기동 시험 **옵트인화**(`8d01995`). ⑵ `udp_control_e2e_test` 의 기본값이 `127.0.0.1:43000`, 즉 **설치 제품**이라 bare 스윕이 사용자 세션에 UDP hello 를 보냈다(17:47:51 거부 4회, 원문 `.claude/thumbnail-timeout-skip/evidence/`) → 기본값 제거·인자 필수(`b941f28`). 무해했던 것은 **운이지 설계가 아니다**. 남은 규칙 6개는 승격이 필요해 손대지 않았다(0.0.21).
- **내 빌드가 내 기계에서만 됐다**: CMakeLists 에 타깃을 등록하고 소스를 미커밋으로 둬 5개 커밋이 clean checkout 에서 configure 실패했다(`7dd0ffc`~`b941f28`, `461b753` 에서 해소). 더 나쁜 것은 내가 "HEAD 는 정상" 이라고 **세 번** 주장한 근거가 **내 워킹트리 대상 configure** 였다는 점이다 — clean checkout 이 아니었다. 재발 방지 둘: 등록된 소스의 **존재**(`7cbcd0a`)와 **git 추적 여부**(`74f8e26`). 후자가 없으면 미추적 파일이 로컬에서 통과하고 남에게서 실패한다.
- **검사가 아무것도 못 찾아도 통과하던 것을 네 번 고쳤다**: 출하 게이트의 미참조 마커(링커가 버려 주입 TU 를 링크해도 통과), 게이트 스캐너의 양성 대조 부재, CMake 검사의 추적 미확인, 그리고 이 문서를 쓰며 붙인 제어문자 검사. 매번 **변이를 걸어보지 않았다면 전부 초록색**이었다.
- 미검증(그대로 남긴다): **설치 제품 실기 0** — picker 증거는 전부 격리 세션의 시험 빌드이고 설치본에서 사람이 본 적은 없다 · UWP·실제 앱 창 캡처 비용 미측정 · lingering 은 **주입 실행만** · 방화벽 Block 6건 · clean checkout 은 **WebView2 SDK 선행 필요**(installer payload 가 `remote60_client_shell` 을 무조건 참조).

### 2026-09-16 클립보드 텍스트 양방향 동기화(K1) — 구현·격리 검증

- **범위**: 뷰어 클립보드 복사→호스트 붙여넣기, 호스트 복사→뷰어 붙여넣기. **텍스트(CF_UNICODETEXT)만**(이미지·파일=K2). 자동 동기화 기본 ON + 뷰어 툴바 on/off 토글.
- **프로토콜(additive·하위호환)**: `MessageType` 3종 `ControlClipboardUpdate/Request/Data`=51/52/53, pong 능력비트 `kCaptureFlagClipboardTextV1`=0x100. 뷰어는 호스트가 이 비트를 광고할 때만 클립보드 메시지를 보낸다 — 구 호스트는 비트를 안 켜 **한 번도 받지 않으므로** 가변길이 payload 오배수(스트림 desync)가 없다. 호스트→뷰어는 뷰어가 보낸 Request의 응답으로만 가고, 구 뷰어는 Request를 안 보낸다. 제어는 strict request/response라 호스트가 unsolicited push를 못 하므로 뷰어가 **generation 기반 폴링**(700ms)으로 가져온다.
- **에코 루프 차단**: 양측이 공유하는 `ClipboardSyncCore`(`clipboard_sync.hpp`)가 FNV-1a `contentHash`로 "방금 보낸 것"·"방금 적용한 것"을 기억한다. 적용은 OS 클립보드에 쓰기 **전에** hash를 기록하므로, 그 쓰기가 유발하는 `WM_CLIPBOARDUPDATE`가 에코로 인식돼 되튕기지 않는다. 부정 대조(두 코어가 텍스트를 주고받아 한 홉 뒤 멈추는지)까지 시험으로 고정.
- **배선**: 호스트(GNLinkStream)는 자체 메시지 펌프가 없어 **메시지 전용 창 + 전용 스레드**의 `ClipboardMonitor`(`clipboard_monitor.*`)로 `AddClipboardFormatListener`를 건다. 뷰어(GNLinkViewer)는 이미 펌프하는 창에 리스너를 붙인다. Serve 루프(`host_control_session.cpp`)에 Update/Request 두 핸들러, 뷰어 제어스레드(`viewer_control_client.cpp` `pump_clipboard_sync`)가 idle turn마다 pending 전송·폴링. 클립보드 쓰기는 창을 소유한 스레드(뷰어 UI thread `kMsgApplyClipboard`)에서만. **영상 경로 무접촉**, LAN=TCP·디렉터리=UDP 터널 공통.
...
### 2026-09-17 인세션 툴바 재디자인 — 상태점, 호스트용 조합키 2개, 그리고 8칸 브러시 캐시

- **재디자인(사용자: "화려하지 않게, 정보 많이 말고 깔끔하게")**: 상태줄을 **예외만 말하도록** 바꿨다. 평소엔 `30fps` 하나. `릴레이`(과금되는 경로)·`입력 꺼짐`·`연결 중` 은 **해당될 때만** 붙는다. `직접`·`경로 확인 중` 표기는 **삭제** — 다만 "확정 전에 직접 연결이라고 주장하지 않는다" 는 원래 약속은 그대로다(둘 다 아무 말도 안 하므로). 건강도는 점이 맡으므로 글자로 중복하지 않는다.
- **상태점(🟢🟠🔴)**: `evaluate_session_health()` 순수 함수. 🔴 제어 끊김 **또는** 영상 기대 중 **2.5초** 무도착 / 🟠 혼잡(수신 복구 중) **또는** RTT>**150ms** / 🟢 그 외. **🔴 가 🟠 를 이긴다** — 죽은 세션을 "느림" 이라 부르는 건 오후를 통째로 낭비시키는 위로다. 부정 대조: 문턱 1µs 아래는 빨강 아님 · **picker 체류는 정지 아님**(영상 기대 안 함) · **측정 안 된 RTT 는 불리하게 쓰지 않음**.
- **호스트용 조합키 2개**: `바탕화면`(Win+D)·`화면전환`(Alt+Tab). 클라 OS 가 먼저 먹어버려 **타이핑으로는 절대 원격에 안 가는** 조합이라 버튼이 유일한 경로다. `host_key_chord()` 순수 함수로 **모디파이어 먼저 내려가고 맨 마지막에 올라온다** 는 순서를 고정 — 먼저 떼면 Alt+Tab 이 맨 Tab 이 되고, 안 떼면 **남의 기계에 Alt 가 눌린 채 남는다**(이 프로젝트가 이미 겪어 접속 시 모디파이어를 털어내는 이유).
  - ⚠️ **안 되는 조건을 적어 둔다**: 호스트가 **포커스 없는 단일 창**을 캡처 중이면 키를 그 창에 PostMessage 하는데, 셸도 작업 전환기도 **posted 메시지를 안 읽는다**. 이 버튼들은 **데스크톱 캡처용**이다.
- **클립보드 토글 제거** — T1 로 스테일 부활이 닫혀 상시 ON 이 안전해졌다. env 킬스위치(`REMOTE60_CLIPBOARD_SYNC=0`)는 유지.
- 🔴 **덤으로 발견한 기존 결함 — 브러시 캐시가 8칸인데 색은 9개였다.** 넘친 색은 조용히 **`values[0]`(배경색)** 으로 그려진다. 즉 **버튼 테두리 하나가 배경색으로 칠해져 보이지 않는 상태**가 이미 있었고, 유효한 브러시라 아무도 오류를 못 봤다. 점 3색을 더해 12색이 되자 **세 상태가 화면에서 똑같이 나와** 비로소 드러났다. 캐시를 24로 키웠다.
- **검증**: `session_toolbar_click_test` **52/52 ×2**. 점은 규칙(순수함수)만이 아니라 **실제 화면에서** 확인 — 상태별로 바를 한 번씩 PrintWindow 해 **x=365 에서 `60C884`/`E8A848`/`E26060`** 세 색이 모두 다름을 단언하고, **초록은 초록답고 빨강은 빨강답다**(R/G 대소)까지 본다. 나머지 7픽셀만 바뀌므로 그 픽셀이 점이 맞다. 조합키는 4스텝 순서 단언. 전 회귀: core 94/94 · wire 46/46 · monitor 12/12 · shared_core · cmake_sources, 호스트/뷰어 clean 빌드.
  - ⚠️ **내 첫 화면 검증은 틀렸다**: 픽셀당 PrintWindow 를 529번 호출했는데 그 사이 바가 **리사이즈**돼 세 행이 같은 그림이 아니었다(429 vs 529). 상태당 **한 번**만 캡처하도록 고쳤다. 그리고 그 직후 `std::min` 이 windows.h 의 `min` 매크로와 충돌해 **빌드가 깨졌는데도 이전 바이너리가 PASS 52 를 찍었다** — 빌드 성공 여부를 먼저 확인하지 않으면 초록색은 거짓말을 한다.
- **미검증**: 실기 — 두 버튼이 실제 호스트에서 바탕화면/창 전환을 일으키는지(호스트 로그의 주입 확인 포함)는 실기 몫. 안드로이드 툴바는 이번 범위 밖.

### 2026-09-17 세션이 튕기던 진짜 이유 — 제어가 끊기면 영상을 보지도 않고 세션을 죽였다

- **증상**: 폰/회사에서 세션이 튕김. 영상은 멀쩡히 디코딩되는데(폰 in=209 out=209) 상향(제어 ack)이 몇 초 비면 릴레이가 `client silent`, 호스트가 `peer-lost`, 그리고 뷰어가 세션을 종료(exit 43)했다.
- 🔴 **원인(코드로 확정)**: `evaluate_session_liveness()` 의 이 가지에 **영상 검사가 아예 없었다**.
  `if (deadSessionUs>0 && controlRequired && controlGone && controlGoneUs>=deadSessionUs) sessionDead=true;`
  `controlRequired = overUdp || controlPort>0` 이라 **실사용 세션은 사실상 전부 true**. 제어 터널이 재전송을 소진해 peer-lost(~6s) → controlGone → **5초 뒤 무조건 사망**, 프레임이 초당 수십 장 들어오고 있어도 그렇다. 이게 사용자가 겪은 "잘 되던 세션에서 쫓겨남" 이다.
  - 기존 테스트는 이 구멍을 못 봤다. `[L6]` 가 "제어 끊김 + 프레임 계속" 을 보긴 하는데 **`controlRequired=false`** 로만 봤다. 실제 값인 `true` 로는 아무도 안 봤다.
- **수정**: 이 가지에 **영상이 발언권**을 준다. 제어가 끊겨도 **영상이 계속 오면 세션을 유지**하고, 영상까지 멈췄을 때만 기존대로 빠르게 종료한다. 대신 **무한정은 아니다** — `controlGoneWithVideoUs`(30s) 천장을 새로 두었다. 제어가 죽은 세션은 입력이 전혀 안 먹으므로 "클릭도 안 되는 그림" 을 영원히 붙잡아 두지는 않는다.
- ⚠️ **이 수정은 과거의 의도적 결정을 뒤집는다.** 기존 시험에 `// video cannot hide dead input control` 이라며 **영상이 있어도 죽이라**는 단언이 있었다. 그 의도(입력 죽은 세션을 방치하지 않기)는 **기한**으로 살렸다 — 영상이 제어의 죽음을 **잠시는** 가릴 수 있지만 30초를 넘기지는 못한다. 단언은 지우지 않고 새 계약으로 고쳐 적었다.
- **상향 keepalive**: 핑 주기 기본값 **1000ms → 500ms**(`kClientControlIntervalMsDefault`). 핑은 지연 측정이 아니라 **"나 살아있다" 는 상향 증거**다 — 릴레이가 몇 초 조용하면 silent 로 보고 끊는다. 상수를 **한 곳**에 두고 Windows 뷰어(`viewer_args`)와 Android(`ClientSessionConnectArgs`)가 **같은 값**을 쓰게 했다(숫자 복사본 두 개가 어긋나는 게 이런 문제의 시작이다).
- **안 건드린 것과 그 이유**: `udp_control_channel.cpp` 의 `kMaxAttempts=24`(~6s)는 **그대로 뒀다.** ⑴ 이 상수는 **호스트도 같이 쓴다**, ⑵ 재전송 소진 경로는 내부 `steady_clock` 이라 주입이 안 돼 **싸게 테스트할 방법이 없다**(수 초 실시간 필요), ⑶ 요청받은 목적(짧은 공백이 세션 종료로 번지지 않기)은 **뷰어 쪽 천장**으로 이미 달성되고 그건 테스트가 있다. transport 를 테스트 없이 넓히는 것보다 낫다고 판단했다.
