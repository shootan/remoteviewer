...
- **A) ack 채널 수명**: Host 가 `SetEvent(ack)` 뒤 **곧바로 `CloseHandle`** 하는데, worker 는 **`PrepareForSwap` 에 가서야** `OpenEventW` 했다. Host 핸들이 마지막이면 **named object 가 그때 소멸**한다 → worker 의 open 실패 → `acked=false` → **정상 승인이 전달됐는데도 abandon.** 그런데 Host 는 이미 나가는 중이다.
  ...
  - ⚠️ **bool seam 만 봐서는 안 잡힌다** — 반환값은 전부 맞았고 **핸들이 사라졌다.** 그래서 회귀를 **실제 named handle 수명**으로 짰다: ⓐ 미리 보유 → Host 가 놓아도 답이 남아 있다 ⓑ **역예시(늦게 열기)** → "late and it was gone" → worker stood down **(Host 는 ExitNow 를 반환한 채로)** ⓒ **채널 없음** → Host 가 `ExitNow` 를 내지 않고 사유를 댄다.
