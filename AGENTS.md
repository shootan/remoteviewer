...
- **검증용 Claude (검토·검사)**: Codex 계획을 독립적으로 검토해 이견·위험·누락은 근거와 함께 Codex 에 수정 요청하고, OK 면 Codex 의 재승인 없이 작업용에 직접 A2A task 로 위임한다. 계획 변경이 필요하다고 판단한 부분은 Codex 의 수정·확정 전에는 작업용에게 위임하지 않는다. 변경 없이 OK 한 독립 범위는 바로 위임할 수 있다. 구현 결과·diff·검증 근거를 독립 검사한다(검증 목적의 빌드/테스트 실행과 검토 기록 작성 가능; 기록은 `.claude/` 등 저장소 미추적 위치). 저장소 추적 파일(제품·문서)을 수정·커밋하지 않으며, 자기 수정물을 자기 승인하지 않는다.
...
- **역할**: 작업용 Claude 가 `gnlink` 권한으로 준비·서명·게시·스크립트 실행까지 한다. 검증용 Claude 는 계획·구현·배포 결과를 검사한다.
  **`gnlink` 권한으로 불가능한 구체 작업만** NAS 세션에 넘기고, 넘길 때는 **정확한 범위·실패 근거·백업/rollback** 을 함께 준다.
  권한이 있는 배포를 NAS 에 중계하거나 매번 재승인받지 않는다. **계정 권한의 광역 완화와 다른 NAS 서비스 변경은 이 정책의 허가 범위가 아니다.**
